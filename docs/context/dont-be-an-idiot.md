# Don't Be An Idiot - Agent SDK V2 API

The `unstable_v2_*` APIs from `@anthropic-ai/claude-agent-sdk` are REAL and WORK despite having "unstable" in the name.

They are in the v1 codebase. They exist. Don't question it. Use them.

## Working Examples

Here is the full content from `docs/context/agent-sdk-v2-examples.ts` demonstrating these APIs in action:
...
- `session.receive()` - Async iterator for responses

These APIs are simpler, cleaner, and less error-prone than the V1 `query()` API.
