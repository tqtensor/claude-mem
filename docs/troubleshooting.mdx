...
```bash
sqlite3 ~/.claude-mem/claude-mem.db "
  SELECT sdk_session_id, type, title, created_at
  FROM observations
  WHERE sdk_session_id = 'YOUR_SESSION_ID'
  ORDER BY created_at;
"
```
...
# Get current worker port from settings
PORT=$(node ~/.claude/plugins/marketplaces/thedotmack/claude-mem/plugin/scripts/settings-cli.js --get workerPort)
