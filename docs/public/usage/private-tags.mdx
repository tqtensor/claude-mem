...
- **Your original prompts** - Tags are stripped before storage in the user_prompts table
...
- Content is filtered at the hook layer before any storage
  - **UserPromptSubmit hook**: Strips tags from user prompts before saving to user_prompts table
  - **PostToolUse hook**: Strips tags from tool inputs and responses before observation creation
...
- Tags are stripped from all content: user prompts, tool inputs, and tool responses
