...
The agent answers from its corpus — no hallucination, no guessing, just your actual project history. Each follow-up question builds on the prior conversation:
...
```
            ...
            (mcp-server.ts)            (worker on :37777)
                  |                          |
   build_corpus ──┤                          |
   list_corpora ──┤── callWorkerAPIPost() ──>|
   prime_corpus ──┤                          |
   query_corpus ──┘                          |
                                             v
                                       CorpusRoutes
