...
v4.0+ auto-starts the worker on first session. Manual commands below are helpful for debugging.

```bash
# Start worker service (auto-starts automatically)
claude-mem start
...
# Start MCP search server (advanced)
claude-mem mcp
```
