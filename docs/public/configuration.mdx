...
├── settings.json           # User settings (env overrides)
...
If port 37777 is in use, you can change it using environment variables or settings file.
...
**Configuration Priority:** Environment variables take precedence over settings file, which takes precedence over defaults.
...
1. Set custom port via environment variable:
   ```bash
   export CLAUDE_MEM_WORKER_PORT=38000
   npm run worker:restart
   ```

   Or via settings file (`~/.claude-mem/settings.json`):
   ```json
   {
     "env": {
       "CLAUDE_MEM_WORKER_PORT": "38000"
...
2. Verify the worker is running on the new port:
   ```bash
   curl http://localhost:38000/api/health
   ```

**Configuration Priority:** Environment variables > Settings file > Defaults (37777)
