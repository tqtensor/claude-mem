...
You can disable memory capture for specific projects by adding them to the `CLAUDE_MEM_IGNORED_PROJECTS` setting in `~/.claude-mem/settings.json`.

### Disabling Memory for Projects

Edit `~/.claude-mem/settings.json` and add a comma-separated list of project names:
...
When a project is in the ignored list, claude-mem will skip all memory capture for that project:
...
- **Client Work**: Disable for projects with compliance requirements

### Viewing Current Settings

You can view your current settings:
```bash
cat ~/.claude-mem/settings.json
```

Or via the web viewer at http://localhost:37777 (Settings > Environment Settings).
