...
<Note>
As of Claude Code 2.1.0 (ultrathink update), SessionStart hooks no longer display any user-visible messages. Context is silently injected via `hookSpecificOutput.additionalContext`.
</Note>
...
<Note>
As of Claude Code 2.1.0, SessionStart hooks output is never displayed to users. Context is injected silently via `hookSpecificOutput.additionalContext`.
</Note>
