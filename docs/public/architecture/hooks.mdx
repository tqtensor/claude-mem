...
<Note>
As of Claude Code 2.1.0 (ultrathink update), SessionStart hooks no longer display any user-visible messages. Context is still injected via `hookSpecificOutput.additionalContext` but users don't see startup output in the UI.
</Note>
