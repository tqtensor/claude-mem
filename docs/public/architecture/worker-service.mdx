...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `CLAUDE_MEM_WORKER_PORT`). The currently running port is also written to `~/.claude-mem/.worker.port`.
...
- **Port File**: `~/.claude-mem/.worker.port` records the currently running port
