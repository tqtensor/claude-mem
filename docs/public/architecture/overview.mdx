...
**Note**: `version-check.js` runs in the Setup phase (not as a lifecycle hook). It performs a sub-100ms read of the `.install-version` marker and, on mismatch, asks the user to run `npx claude-mem repair`. The actual runtime install (Bun, uv, `bun install`) is performed by `npx claude-mem install` / `npx claude-mem repair` with a visible clack spinner.
