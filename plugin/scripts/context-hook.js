#!/usr/bin/env node
import M from"path";import{homedir as ye}from"os";import{existsSync as Ae,readFileSync as ke,unlinkSync as Ce}from"fs";import{stdin as J}from"process";import{fileURLToPath as De}from"url";import{dirname as xe}from"path";import Re from"better-sqlite3";import{join as f,dirname as me,basename as qe}from"path";import{homedir as se}from"os";import{existsSync as ze,mkdirSync as Ee}from"fs";import{fileURLToPath as Te}from"url";function he(){return typeof __dirname<"u"?__dirname:me(Te(import.meta.url))}var es=he(),y=process.env.CLAUDE_MEM_DATA_DIR||f(se(),".claude-mem"),W=process.env.CLAUDE_CONFIG_DIR||f(se(),".claude"),ss=f(y,"archives"),ts=f(y,"logs"),rs=f(y,"trash"),ns=f(y,"backups"),os=f(y,"settings.json"),te=f(y,"claude-mem.db"),is=f(y,"vector-db"),as=f(W,"settings.json"),ds=f(W,"commands"),cs=f(W,"CLAUDE.md");function re(d){Ee(d,{recursive:!0})}var Y=(a=>(a[a.DEBUG=0]="DEBUG",a[a.INFO=1]="INFO",a[a.WARN=2]="WARN",a[a.ERROR=3]="ERROR",a[a.SILENT=4]="SILENT",a))(Y||{}),K=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=Y[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,a){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),c=Y[e].padEnd(5),_=s.padEnd(6),h="";r?.correlationId?h=`[${r.correlationId}] `:r?.sessionId&&(h=`[session-${r.sessionId}] `);let n="";a!=null&&(this.level===0&&typeof a=="object"?n=`
`+JSON.stringify(a,null,2):n=" "+this.formatData(a));let R="";if(r){let{sessionId:g,sdkSessionId:S,correlationId:E,...p}=r;Object.keys(p).length>0&&(R=` {${Object.entries(p).map(([H,j])=>`${H}=${j}`).join(", ")}}`)}let N=`[${i}] [${c}] [${_}] ${h}${t}${R}${n}`;e===3?console.error(N):console.log(N)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},k=new K;import{appendFileSync as be}from"fs";import{homedir as ge}from"os";import{join as Se}from"path";var fe=Se(ge(),".pm2","logs","claude-mem-worker-error.log");function l(d,e,s=""){let t=new Date().toISOString(),c=((new Error().stack?.split(`
`)??[])[2]??"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),_=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",h=`[${t}] [HAPPY-PATH-ERROR] [${_}] ${d}`;if(e!==void 0)try{h+=` ${JSON.stringify(e)}`}catch(n){h+=` [stringify error: ${n}]`}h+=`
`;try{be(fe,h)}catch{}return s}var w=class{db;constructor(){re(y),this.db=new Re(te),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn(),this.removeToolUseIdUniqueConstraint()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)?.worker_port||l("SessionStore.getWorkerPort: worker_port is null",{id:e},null)}saveUserPrompt(e,s,t){let r=new Date,a=r.getTime();return this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(e,s,t,r.toISOString(),a).lastInsertRowid}getUserPrompt(e,s){return this.db.prepare(`
      SELECT prompt_text
      FROM user_prompts
      WHERE claude_session_id = ? AND prompt_number = ?
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,t,r,a=0){let i=new Date,c=i.getTime();this.db.prepare(`
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,a=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",c=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${i}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,a){let i=a?"AND project = ?":"",c=a?[a]:[],_,h;if(e!==null){let g=`
      ...
      `,S=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let E=this.db.prepare(g).all(e,...c,t+1),p=this.db.prepare(S).all(e,...c,r+1);if(E.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=E.length>0?E[E.length-1].created_at_epoch:s,h=p.length>0?p[p.length-1].created_at_epoch:s}catch(E){return console.error("[SessionStore] Error getting boundary observations:",E.message),{observations:[],sessions:[],prompts:[]}}}else{let g=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let E=this.db.prepare(g).all(s,...c,t),p=this.db.prepare(S).all(s,...c,r+1);if(E.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=E.length>0?E[E.length-1].created_at_epoch:s,h=p.length>0?p[p.length-1].created_at_epoch:s}catch(E){return console.error("[SessionStore] Error getting boundary timestamps:",E.message),{observations:[],sessions:[],prompts:[]}}}let n=`
      SELECT *
      FROM observations
    ...
    `).run(s,t,r,e)}getEndlessModeStats(e){let t=this.db.prepare(`
      ...
      FROM sdk_sessions
      WHERE claude_session_id = ?
    `).get(e);return t?{originalTokens:t.endless_original_tokens||l("SessionStore.getEndlessModeStats: endless_original_tokens is null",{claudeSessionId:e},0),compressedTokens:t.endless_compressed_tokens||l("SessionStore.getEndlessModeStats: endless_compressed_tokens is null",{claudeSessionId:e},0),tokensSaved:t.endless_tokens_saved||l("SessionStore.getEndlessModeStats: endless_tokens_saved is null",{claudeSessionId:e},0)}:null}close(){this.db.close()}};import{existsSync as Oe,readFileSync as Ne}from"fs";import{homedir as Ie}from"os";import ve from"path";function q(d,e,s){if(d!==void 0){if(typeof d=="boolean")return d;if(typeof d=="string")return d.toLowerCase()==="true"}return e!==void 0?e.toLowerCase()==="true":s}function V(d,e,s){if(d!==void 0){if(typeof d=="number")return d;if(typeof d=="string"){let t=parseInt(d,10);if(!isNaN(t))return t}}if(e!==void 0){let t=parseInt(e,10);if(!isNaN(t))return t}return s}function Le(){let d=ve.join(Ie(),".claude-mem","settings.json"),e={};if(Oe(d))try{e=JSON.parse(Ne(d,"utf-8"))}catch(h){k.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},h)}let s=q(e.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),t=q(e.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),r=V(e.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),a=V(e.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),i=V(e.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50),c=q(e.env?.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,process.env.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,s),_={enabled:s,fallbackToOriginal:t,maxLookupTime:r,keepRecentToolUses:a,maxToolHistoryMB:i,enableSynchronousMode:c};return s?k.info("CONFIG","Endless Mode enabled",{fallback:t,maxLookupTime:`${r}ms`,keepRecent:a,maxToolHistoryMB:`${i}MB`,syncMode:c}):k.debug("CONFIG","Endless Mode disabled"),_}var F=class{static getConfig=Le;static clearCache(){}};var Me=De(import.meta.url),Ue=xe(Me),$e=M.join(Ue,"../../.install-version");function we(){try{let d=M.join(ye(),".claude","settings.json");if(Ae(d)){let e=JSON.parse(ke(d,"utf-8"));if(e.env?.CLAUDE_MEM_CONTEXT_OBSERVATIONS){let s=parseInt(e.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10);if(!isNaN(s)&&s>0)return s}}}catch{}return parseInt(process.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS||l("context-hook: CLAUDE_MEM_CONTEXT_OBSERVATIONS not set",{},"50"),10)}var Fe=we(),ne=10,oe=4,Xe=1,o={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function Be(d){if(!d)return[];try{let e=JSON.parse(d);return Array.isArray(e)?e:[]}catch{return[]}}function He(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function je(d){return new Date(d).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Pe(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function Ge(d,e){return M.isAbsolute(d)?M.relative(e,d):d}function X(d,e,s,t){return e?t?[`${s}${d}:${o.reset} ${e}`,""]:[`**${d}**: ${e}`,""]:[]}async function ie(d,e=!1){let s=d?.cwd??process.cwd(),t=s?M.basename(s):"unknown-project",r;try{r=new w}catch(g){if(g.code==="ERR_DLOPEN_FAILED"){try{Ce($e)}catch{}console.error("\u26A0\uFE0F  Native module rebuild needed - restart Claude Code to auto-fix"),console.error("   (This happens after Node.js version upgrades)"),process.exit(0)}throw g}let a=r.db.prepare(`
    SELECT
...
No previous sessions found for this project yet.`;let c=a,_=i.slice(0,ne),h=c,n=[];if(e?(n.push(""),n.push(`${o.bright}${o.cyan}\u{1F4DD} [${t}] recent context${o.reset}`),n.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),n.push("")):(n.push(`# [${t}] recent context`),n.push("")),h.length>0){e?n.push(`${o.dim}Legend: \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision${o.reset}`):n.push("**Legend:** \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision"),n.push(""),e?(n.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),n.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),n.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(n.push("\u{1F4A1} **Column Key**:"),n.push("- **Read**: Tokens to read this observation (cost to learn it now)"),n.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),n.push(""),e?(n.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),n.push(""),n.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),n.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),n.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),n.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(n.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),n.push(""),n.push("When you need implementation details, rationale, or debugging context:"),n.push("- Use the mem-search skill to fetch full observations on-demand"),n.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),n.push("- Trust this index over re-reading code for past decisions and learnings")),n.push("");let g=c.length,S=c.reduce((u,T)=>{let b=(T.title?.length||l("context-hook: obs.title.length is null",{obsId:T.id},0))+(T.subtitle?.length||l("context-hook: obs.subtitle.length is null",{obsId:T.id},0))+(T.narrative?.length||l("context-hook: obs.narrative.length is null",{obsId:T.id},0))+JSON.stringify(T.facts||l("context-hook: obs.facts is null",{obsId:T.id},[])).length;return u+Math.ceil(b/oe)},0),E=c.reduce((u,T)=>u+(T.discovery_tokens||l("context-hook: obs.discovery_tokens is null",{obsId:T.id},0)),0),p=E-S,B=E>0?Math.round(p/E*100):0;e?(n.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),n.push(`${o.dim}  Loading: ${g} observations (${S.toLocaleString()} tokens to read)${o.reset}`),n.push(`${o.dim}  Work investment: ${E.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),E>0&&n.push(`${o.green}  Your savings: ${p.toLocaleString()} tokens (${B}% reduction from reuse)${o.reset}`),n.push("")):(n.push("\u{1F4CA} **Context Economics**:"),n.push(`- Loading: ${g} observations (${S.toLocaleString()} tokens to read)`),n.push(`- Work investment: ${E.toLocaleString()} tokens spent on research, building, and decisions`),E>0&&n.push(`- Your savings: ${p.toLocaleString()} tokens (${B}% reduction from reuse)`),n.push(""));let H=i[0]?.id,j=_.map((u,T)=>{let b=T===0?null:i[T+1];return{...u,displayEpoch:b?b.created_at_epoch:u.created_at_epoch,displayTime:b?b.created_at:u.created_at,shouldShowLink:u.id!==H}}),Q=[...h.map(u=>({type:"observation",data:u})),...j.map(u=>({type:"summary",data:u}))];Q.sort((u,T)=>{let b=u.type==="observation"?u.data.created_at_epoch:u.data.displayEpoch,A=T.type==="observation"?T.data.created_at_epoch:T.data.displayEpoch;return b-A});let U=new Map;for(let u of Q){let T=u.type==="observation"?u.data.created_at:u.data.displayTime,b=Pe(T);U.has(b)||U.set(b,[]),U.get(b).push(u)}let ae=Array.from(U.entries()).sort((u,T)=>{let b=new Date(u[0]).getTime(),A=new Date(T[0]).getTime();return b-A});for(let[u,T]of ae){e?(n.push(`${o.bright}${o.cyan}${u}${o.reset}`),n.push("")):(n.push(`### ${u}`),n.push(""));let b=null,A="",C=!1;for(let P of T)if(P.type==="summary"){C&&(n.push(""),C=!1,b=null,A="");let m=P.data,D=`${m.request||l("context-hook: summary.request is null",{summaryId:m.id},"Session started")} (${He(m.displayTime)})`,I=m.shouldShowLink?`claude-mem://session-summary/${m.id}`:"";if(e){let v=I?`${o.dim}[${I}]${o.reset}`:"";n.push(`\u{1F3AF} ${o.yellow}#S${m.id}${o.reset} ${D} ${v}`)}else{let v=I?` [\u2192](${I})`:"";n.push(`**\u{1F3AF} #S${m.id}** ${D}${v}`)}n.push("")}else{let m=P.data,D=Be(m.files_modified),I=D.length>0?Ge(D[0],s):"General";I!==b&&(C&&n.push(""),e?n.push(`${o.dim}${I}${o.reset}`):n.push(`**${I}**`),e||(n.push("| ID | Time | T | Title | Read | Work |"),n.push("|----|------|---|-------|------|------|")),b=I,C=!0,A="");let v=je(m.created_at),Z=m.title||l("context-hook: obs.title is null",{obsId:m.id},"Untitled"),L="\u2022";switch(m.type){case"bugfix":L="\u{1F534}";break;case"feature":L="\u{1F7E3}";break;case"refactor":L="\u{1F504}";break;case"change":L="\u2705";break;case"discovery":L="\u{1F535}";break;case"decision":L="\u2696\uFE0F";break;default:L="\u2022"}let de=(m.title?.length||l("context-hook: obs.title.length is null (timeline)",{obsId:m.id},0))+(m.subtitle?.length||l("context-hook: obs.subtitle.length is null (timeline)",{obsId:m.id},0))+(m.narrative?.length||l("context-hook: obs.narrative.length is null (timeline)",{obsId:m.id},0))+JSON.stringify(m.facts||l("context-hook: obs.facts is null (timeline)",{obsId:m.id},[])).length,G=Math.ceil(de/oe),$=m.discovery_tokens||l("context-hook: obs.discovery_tokens is null (timeline)",{obsId:m.id},0),x="\u{1F50D}";switch(m.type){case"discovery":x="\u{1F50D}";break;case"change":case"feature":case"bugfix":case"refactor":x="\u{1F6E0}\uFE0F";break;case"decision":x="\u2696\uFE0F";break}let ce=$>0?`${x} ${$.toLocaleString()}`:"-",ee=v!==A,pe=ee?v:"";if(A=v,e){let le=ee?`${o.dim}${v}${o.reset}`:" ".repeat(v.length),_e=G>0?`${o.dim}(~${G}t)${o.reset}`:"",ue=$>0?`${o.dim}(${x} ${$.toLocaleString()}t)${o.reset}`:"";n.push(`  ${o.dim}#${m.id}${o.reset}  ${le}  ${L}  ${Z} ${_e} ${ue}`)}else n.push(`| #${m.id} | ${pe||l("context-hook: timeDisplay is null",{obsId:m.id},"\u2033")} | ${L} | ${Z} | ~${G} | ${ce} |`)}C&&n.push("")}let O=i[0],z=c[0];if(O&&(O.investigated||O.learned||O.completed||O.next_steps)&&(!z||O.created_at_epoch>z.created_at_epoch)&&(n.push(...X("Investigated",O.investigated,o.blue,e)),n.push(...X("Learned",O.learned,o.yellow,e)),n.push(...X("Completed",O.completed,o.green,e)),n.push(...X("Next Steps",O.next_steps,o.magenta,e))),E>0&&p>0){let u=Math.round(E/1e3);n.push(""),e?n.push(`${o.dim}\u{1F4B0} Access ${u}k tokens of past research & decisions for just ${S.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${o.reset}`):n.push(`\u{1F4B0} Access ${u}k tokens of past research & decisions for just ${S.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}let R=F.getConfig();n.push(""),n.push("");let N=R.enabled?"On":"Off";if(e){let g=R.enabled?o.green:o.dim;n.push(`${o.bright}${o.magenta}\u{1F504} Endless Mode:${o.reset} ${g}${N}${o.reset}`)}else n.push(`\u{1F504} **Endless Mode:** ${N}`);return r.close(),n.join(`
`).trimEnd()}var We=process.argv.includes("--colors");if(J.isTTY||We)ie(void 0,!0).then(d=>{console.log(d),process.exit(0)});else{let d="";J.on("data",e=>d+=e),J.on("end",async()=>{let e=d.trim()?JSON.parse(d):void 0,t={hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:await ie(e,!1)}};console.log(JSON.stringify(t)),process.exit(0)})}
