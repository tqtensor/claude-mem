#!/usr/bin/env node
import X from"path";import{homedir as Re}from"os";import{existsSync as Ne,readFileSync as Oe}from"fs";import{stdin as q}from"process";import Te from"better-sqlite3";import{join as f,dirname as _e,basename as Ue}from"path";import{homedir as Z}from"os";import{existsSync as Xe,mkdirSync as ue}from"fs";import{fileURLToPath as me}from"url";function Ee(){return typeof __dirname<"u"?__dirname:_e(me(import.meta.url))}var je=Ee(),L=process.env.CLAUDE_MEM_DATA_DIR||f(Z(),".claude-mem"),W=process.env.CLAUDE_CONFIG_DIR||f(Z(),".claude"),Ge=f(L,"archives"),We=f(L,"logs"),Pe=f(L,"trash"),Ye=f(L,"backups"),Ke=f(L,"settings.json"),ee=f(L,"claude-mem.db"),qe=f(L,"vector-db"),Ve=f(W,"settings.json"),Je=f(W,"commands"),Qe=f(W,"CLAUDE.md");function se(d){ue(d,{recursive:!0})}var P=(a=>(a[a.DEBUG=0]="DEBUG",a[a.INFO=1]="INFO",a[a.WARN=2]="WARN",a[a.ERROR=3]="ERROR",a[a.SILENT=4]="SILENT",a))(P||{}),Y=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=P[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,a){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),c=P[e].padEnd(5),_=s.padEnd(6),b="";r?.correlationId?b=`[${r.correlationId}] `:r?.sessionId&&(b=`[session-${r.sessionId}] `);let n="";a!=null&&(this.level===0&&typeof a=="object"?n=`
`+JSON.stringify(a,null,2):n=" "+this.formatData(a));let N="";if(r){let{sessionId:g,sdkSessionId:S,correlationId:m,...p}=r;Object.keys(p).length>0&&(N=` {${Object.entries(p).map(([H,M])=>`${H}=${M}`).join(", ")}}`)}let u=`[${i}] [${c}] [${_}] ${b}${t}${N}${n}`;e===3?console.error(u):console.log(u)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},A=new Y;var U=class{db;constructor(){se(L),this.db=new Te(ee),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,a=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",c=e.map(()=>"?").join(",");return this.db.prepare(`
      SELECT *
      FROM observations
      WHERE id IN (${c})
    ...
    `).get(e)?.prompt_counter||1}getPromptCounter(e){return this.db.prepare(`
      SELECT prompt_counter FROM sdk_sessions WHERE id = ?
    `).get(e)?.prompt_counter||0}createSDKSession(e,s,t){let r=new Date,a=r.getTime(),c=this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),r||null,a,t.tool_use_id||null,i.toISOString(),c);return{id:Number(N.lastInsertRowid),createdAtEpoch:c}}getObservationByToolUseId(e){return this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||null,a,i.toISOString(),c);return{id:Number(N.lastInsertRowid),createdAtEpoch:c}}markSessionCompleted(e){let s=new Date,t=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,a=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",c=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${i}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,a){let i=a?"AND project = ?":"",c=a?[a]:[],_,b;if(e!==null){let g=`
      ...
      `,S=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let m=this.db.prepare(g).all(e,...c,t+1),p=this.db.prepare(S).all(e,...c,r+1);if(m.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=m.length>0?m[m.length-1].created_at_epoch:s,b=p.length>0?p[p.length-1].created_at_epoch:s}catch(m){return console.error("[SessionStore] Error getting boundary observations:",m.message),{observations:[],sessions:[],prompts:[]}}}else{let g=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let m=this.db.prepare(g).all(s,...c,t),p=this.db.prepare(S).all(s,...c,r+1);if(m.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=m.length>0?m[m.length-1].created_at_epoch:s,b=p.length>0?p[p.length-1].created_at_epoch:s}catch(m){return console.error("[SessionStore] Error getting boundary timestamps:",m.message),{observations:[],sessions:[],prompts:[]}}}let n=`
      SELECT *
      FROM observations
    ...
    `).run(s,t,r,e)}getEndlessModeStats(e){let t=this.db.prepare(`
      ...
      FROM sdk_sessions
      WHERE claude_session_id = ?
    `).get(e);return t?{originalTokens:t.endless_original_tokens||0,compressedTokens:t.endless_compressed_tokens||0,tokensSaved:t.endless_tokens_saved||0}:null}close(){this.db.close()}};import{existsSync as ge,readFileSync as he}from"fs";import{homedir as be}from"os";import Se from"path";function w(d,e,s){if(d!==void 0){if(typeof d=="boolean")return d;if(typeof d=="string")return d.toLowerCase()==="true"}return e!==void 0?e.toLowerCase()==="true":s}function K(d,e,s){if(d!==void 0){if(typeof d=="number")return d;if(typeof d=="string"){let t=parseInt(d,10);if(!isNaN(t))return t}}if(e!==void 0){let t=parseInt(e,10);if(!isNaN(t))return t}return s}function fe(){let d=Se.join(be(),".claude-mem","settings.json"),e={};if(ge(d))try{e=JSON.parse(he(d,"utf-8"))}catch(n){A.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},n)}let s=w(e.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),t=w(e.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),r=K(e.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),a=K(e.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),i=w(e.env?.CLAUDE_MEM_OBSERVE_EVERYTHING,process.env.CLAUDE_MEM_OBSERVE_EVERYTHING,s),c=K(e.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50),_=w(e.env?.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,process.env.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,s),b={enabled:s,fallbackToOriginal:t,maxLookupTime:r,keepRecentToolUses:a,observeEverything:i,maxToolHistoryMB:c,enableSynchronousMode:_};return s?A.info("CONFIG","Endless Mode enabled",{fallback:t,maxLookupTime:`${r}ms`,keepRecent:a,observeEverything:i,maxToolHistoryMB:`${c}MB`}):A.debug("CONFIG","Endless Mode disabled"),b}var F=class{static getConfig=fe;static clearCache(){}};function Ie(){try{let d=X.join(Re(),".claude","settings.json");if(Ne(d)){let e=JSON.parse(Oe(d,"utf-8"));if(e.env?.CLAUDE_MEM_CONTEXT_OBSERVATIONS){let s=parseInt(e.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10);if(!isNaN(s)&&s>0)return s}}}catch{}return parseInt(process.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS||"50",10)}var ve=Ie(),te=10,re=4,Le=1,o={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function ye(d){if(!d)return[];try{let e=JSON.parse(d);return Array.isArray(e)?e:[]}catch{return[]}}function Ae(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ke(d){return new Date(d).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Ce(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function De(d,e){return X.isAbsolute(d)?X.relative(e,d):d}function B(d,e,s,t){return e?t?[`${s}${d}:${o.reset} ${e}`,""]:[`**${d}**: ${e}`,""]:[]}async function ne(d,e=!1){let s=d?.cwd??process.cwd(),t=s?X.basename(s):"unknown-project",r=new U,a=r.db.prepare(`
    SELECT
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,ve),i=r.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,te+Le);if(a.length===0&&i.length===0)return r.close(),e?`
...
No previous sessions found for this project yet.`;let c=a,_=i.slice(0,te),b=c,n=[];if(e?(n.push(""),n.push(`${o.bright}${o.cyan}\u{1F4DD} [${t}] recent context${o.reset}`),n.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),n.push("")):(n.push(`# [${t}] recent context`),n.push("")),b.length>0){e?n.push(`${o.dim}Legend: \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision${o.reset}`):n.push("**Legend:** \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision"),n.push(""),e?(n.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),n.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),n.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(n.push("\u{1F4A1} **Column Key**:"),n.push("- **Read**: Tokens to read this observation (cost to learn it now)"),n.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),n.push(""),e?(n.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),n.push(""),n.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),n.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),n.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),n.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(n.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),n.push(""),n.push("When you need implementation details, rationale, or debugging context:"),n.push("- Use the mem-search skill to fetch full observations on-demand"),n.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),n.push("- Trust this index over re-reading code for past decisions and learnings")),n.push("");let u=c.length,g=c.reduce((l,E)=>{let h=(E.title?.length||0)+(E.subtitle?.length||0)+(E.narrative?.length||0)+JSON.stringify(E.facts||[]).length;return l+Math.ceil(h/re)},0),S=c.reduce((l,E)=>l+(E.discovery_tokens||0),0),m=S-g,p=S>0?Math.round(m/S*100):0;e?(n.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),n.push(`${o.dim}  Loading: ${u} observations (${g.toLocaleString()} tokens to read)${o.reset}`),n.push(`${o.dim}  Work investment: ${S.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),S>0&&n.push(`${o.green}  Your savings: ${m.toLocaleString()} tokens (${p}% reduction from reuse)${o.reset}`),n.push("")):(n.push("\u{1F4CA} **Context Economics**:"),n.push(`- Loading: ${u} observations (${g.toLocaleString()} tokens to read)`),n.push(`- Work investment: ${S.toLocaleString()} tokens spent on research, building, and decisions`),S>0&&n.push(`- Your savings: ${m.toLocaleString()} tokens (${p}% reduction from reuse)`),n.push(""));let V=i[0]?.id,H=_.map((l,E)=>{let h=E===0?null:i[E+1];return{...l,displayEpoch:h?h.created_at_epoch:l.created_at_epoch,displayTime:h?h.created_at:l.created_at,shouldShowLink:l.id!==V}}),M=[...b.map(l=>({type:"observation",data:l})),...H.map(l=>({type:"summary",data:l}))];M.sort((l,E)=>{let h=l.type==="observation"?l.data.created_at_epoch:l.data.displayEpoch,y=E.type==="observation"?E.data.created_at_epoch:E.data.displayEpoch;return h-y});let x=new Map;for(let l of M){let E=l.type==="observation"?l.data.created_at:l.data.displayTime,h=Ce(E);x.has(h)||x.set(h,[]),x.get(h).push(l)}let oe=Array.from(x.entries()).sort((l,E)=>{let h=new Date(l[0]).getTime(),y=new Date(E[0]).getTime();return h-y});for(let[l,E]of oe){e?(n.push(`${o.bright}${o.cyan}${l}${o.reset}`),n.push("")):(n.push(`### ${l}`),n.push(""));let h=null,y="",k=!1;for(let j of E)if(j.type==="summary"){k&&(n.push(""),k=!1,h=null,y="");let T=j.data,C=`${T.request||"Session started"} (${Ae(T.displayTime)})`,O=T.shouldShowLink?`claude-mem://session-summary/${T.id}`:"";if(e){let I=O?`${o.dim}[${O}]${o.reset}`:"";n.push(`\u{1F3AF} ${o.yellow}#S${T.id}${o.reset} ${C} ${I}`)}else{let I=O?` [\u2192](${O})`:"";n.push(`**\u{1F3AF} #S${T.id}** ${C}${I}`)}n.push("")}else{let T=j.data,C=ye(T.files_modified),O=C.length>0?De(C[0],s):"General";O!==h&&(k&&n.push(""),e?n.push(`${o.dim}${O}${o.reset}`):n.push(`**${O}**`),e||(n.push("| ID | Time | T | Title | Read | Work |"),n.push("|----|------|---|-------|------|------|")),h=O,k=!0,y="");let I=ke(T.created_at),Q=T.title||"Untitled",v="\u2022";switch(T.type){case"bugfix":v="\u{1F534}";break;case"feature":v="\u{1F7E3}";break;case"refactor":v="\u{1F504}";break;case"change":v="\u2705";break;case"discovery":v="\u{1F535}";break;case"decision":v="\u2696\uFE0F";break;default:v="\u2022"}let ie=(T.title?.length||0)+(T.subtitle?.length||0)+(T.narrative?.length||0)+JSON.stringify(T.facts||[]).length,G=Math.ceil(ie/re),$=T.discovery_tokens||0,D="\u{1F50D}";switch(T.type){case"discovery":D="\u{1F50D}";break;case"change":case"feature":case"bugfix":case"refactor":D="\u{1F6E0}\uFE0F";break;case"decision":D="\u2696\uFE0F";break}let ae=$>0?`${D} ${$.toLocaleString()}`:"-",z=I!==y,de=z?I:"";if(y=I,e){let ce=z?`${o.dim}${I}${o.reset}`:" ".repeat(I.length),pe=G>0?`${o.dim}(~${G}t)${o.reset}`:"",le=$>0?`${o.dim}(${D} ${$.toLocaleString()}t)${o.reset}`:"";n.push(`  ${o.dim}#${T.id}${o.reset}  ${ce}  ${v}  ${Q} ${pe} ${le}`)}else n.push(`| #${T.id} | ${de||"\u2033"} | ${v} | ${Q} | ~${G} | ${ae} |`)}k&&n.push("")}let R=i[0],J=c[0];if(R&&(R.investigated||R.learned||R.completed||R.next_steps)&&(!J||R.created_at_epoch>J.created_at_epoch)&&(n.push(...B("Investigated",R.investigated,o.blue,e)),n.push(...B("Learned",R.learned,o.yellow,e)),n.push(...B("Completed",R.completed,o.green,e)),n.push(...B("Next Steps",R.next_steps,o.magenta,e))),S>0&&m>0){let l=Math.round(S/1e3);n.push(""),e?n.push(`${o.dim}\u{1F4B0} Access ${l}k tokens of past research & decisions for just ${g.toLocaleString()}t. Use claude-mem search to access memories by ID instead of re-reading files.${o.reset}`):n.push(`\u{1F4B0} Access ${l}k tokens of past research & decisions for just ${g.toLocaleString()}t. Use claude-mem search to access memories by ID instead of re-reading files.`)}}if(F.getConfig().enabled){let u=r.db.prepare(`
...
`).trimEnd()}var Me=process.argv.includes("--colors");if(q.isTTY||Me)ne(void 0,!0).then(d=>{console.log(d),process.exit(0)});else{let d="";q.on("data",e=>d+=e),q.on("end",async()=>{let e=d.trim()?JSON.parse(d):void 0,t={hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:await ne(e,!1)}};console.log(JSON.stringify(t)),process.exit(0)})}
