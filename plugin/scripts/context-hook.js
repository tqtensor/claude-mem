...
`+JSON.stringify(a,null,2):n=" "+this.formatData(a));let N="";if(r){let{sessionId:g,sdkSessionId:b,correlationId:m,...p}=r;Object.keys(p).length>0&&(N=` {${Object.entries(p).map(([X,x])=>`${X}=${x}`).join(", ")}}`)}let u=`[${i}] [${d}] [${_}] ${S}${t}${N}${n}`;e===3?console.error(u):console.log(u)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},A=new P;var U=class{db;constructor(){Z(L),this.db=new me(z),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,a=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",d=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${i}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,a){let i=a?"AND project = ?":"",d=a?[a]:[],_,S;if(e!==null){let g=`
      ...
      `,b=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let m=this.db.prepare(g).all(e,...d,t+1),p=this.db.prepare(b).all(e,...d,r+1);if(m.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=m.length>0?m[m.length-1].created_at_epoch:s,S=p.length>0?p[p.length-1].created_at_epoch:s}catch(m){return console.error("[SessionStore] Error getting boundary observations:",m.message),{observations:[],sessions:[],prompts:[]}}}else{let g=`
      ...
      `,b=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let m=this.db.prepare(g).all(s,...d,t),p=this.db.prepare(b).all(s,...d,r+1);if(m.length===0&&p.length===0)return{observations:[],sessions:[],prompts:[]};_=m.length>0?m[m.length-1].created_at_epoch:s,S=p.length>0?p[p.length-1].created_at_epoch:s}catch(m){return console.error("[SessionStore] Error getting boundary timestamps:",m.message),{observations:[],sessions:[],prompts:[]}}}let n=`
      SELECT *
      FROM observations
...
No previous sessions found for this project yet.`;let d=a,_=i.slice(0,ee),S=d,n=[];if(e?(n.push(""),n.push(`${o.bright}${o.cyan}\u{1F4DD} [${t}] recent context${o.reset}`),n.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),n.push("")):(n.push(`# [${t}] recent context`),n.push("")),S.length>0){e?n.push(`${o.dim}Legend: \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision${o.reset}`):n.push("**Legend:** \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision"),n.push(""),e?(n.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),n.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),n.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(n.push("\u{1F4A1} **Column Key**:"),n.push("- **Read**: Tokens to read this observation (cost to learn it now)"),n.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),n.push(""),e?(n.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),n.push(""),n.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),n.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),n.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),n.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(n.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),n.push(""),n.push("When you need implementation details, rationale, or debugging context:"),n.push("- Use the mem-search skill to fetch full observations on-demand"),n.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),n.push("- Trust this index over re-reading code for past decisions and learnings")),n.push("");let u=d.length,g=d.reduce((l,E)=>{let h=(E.title?.length||0)+(E.subtitle?.length||0)+(E.narrative?.length||0)+JSON.stringify(E.facts||[]).length;return l+Math.ceil(h/se)},0),b=d.reduce((l,E)=>l+(E.discovery_tokens||0),0),m=b-g,p=b>0?Math.round(m/b*100):0;e?(n.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),n.push(`${o.dim}  Loading: ${u} observations (${g.toLocaleString()} tokens to read)${o.reset}`),n.push(`${o.dim}  Work investment: ${b.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),b>0&&n.push(`${o.green}  Your savings: ${m.toLocaleString()} tokens (${p}% reduction from reuse)${o.reset}`),n.push("")):(n.push("\u{1F4CA} **Context Economics**:"),n.push(`- Loading: ${u} observations (${g.toLocaleString()} tokens to read)`),n.push(`- Work investment: ${b.toLocaleString()} tokens spent on research, building, and decisions`),b>0&&n.push(`- Your savings: ${m.toLocaleString()} tokens (${p}% reduction from reuse)`),n.push(""));let K=i[0]?.id,X=_.map((l,E)=>{let h=E===0?null:i[E+1];return{...l,displayEpoch:h?h.created_at_epoch:l.created_at_epoch,displayTime:h?h.created_at:l.created_at,shouldShowLink:l.id!==K}}),x=[...S.map(l=>({type:"observation",data:l})),...X.map(l=>({type:"summary",data:l}))];x.sort((l,E)=>{let h=l.type==="observation"?l.data.created_at_epoch:l.data.displayEpoch,y=E.type==="observation"?E.data.created_at_epoch:E.data.displayEpoch;return h-y});let $=new Map;for(let l of x){let E=l.type==="observation"?l.data.created_at:l.data.displayTime,h=ye(E);$.has(h)||$.set(h,[]),$.get(h).push(l)}let re=Array.from($.entries()).sort((l,E)=>{let h=new Date(l[0]).getTime(),y=new Date(E[0]).getTime();return h-y});for(let[l,E]of re){e?(n.push(`${o.bright}${o.cyan}${l}${o.reset}`),n.push("")):(n.push(`### ${l}`),n.push(""));let h=null,y="",k=!1;for(let j of E)if(j.type==="summary"){k&&(n.push(""),k=!1,h=null,y="");let T=j.data,C=`${T.request||"Session started"} (${ve(T.displayTime)})`,O=T.shouldShowLink?`claude-mem://session-summary/${T.id}`:"";if(e){let I=O?`${o.dim}[${O}]${o.reset}`:"";n.push(`\u{1F3AF} ${o.yellow}#S${T.id}${o.reset} ${C} ${I}`)}else{let I=O?` [\u2192](${O})`:"";n.push(`**\u{1F3AF} #S${T.id}** ${C}${I}`)}n.push("")}else{let T=j.data,C=Ie(T.files_modified),O=C.length>0?Ae(C[0],s):"General";O!==h&&(k&&n.push(""),e?n.push(`${o.dim}${O}${o.reset}`):n.push(`**${O}**`),e||(n.push("| ID | Time | T | Title | Read | Work |"),n.push("|----|------|---|-------|------|------|")),h=O,k=!0,y="");let I=Le(T.created_at),V=T.title||"Untitled",v="\u2022";switch(T.type){case"bugfix":v="\u{1F534}";break;case"feature":v="\u{1F7E3}";break;case"refactor":v="\u{1F504}";break;case"change":v="\u2705";break;case"discovery":v="\u{1F535}";break;case"decision":v="\u2696\uFE0F";break;default:v="\u2022"}let ne=(T.title?.length||0)+(T.subtitle?.length||0)+(T.narrative?.length||0)+JSON.stringify(T.facts||[]).length,H=Math.ceil(ne/se),M=T.discovery_tokens||0,D="\u{1F50D}";switch(T.type){case"discovery":D="\u{1F50D}";break;case"change":case"feature":case"bugfix":case"refactor":D="\u{1F6E0}\uFE0F";break;case"decision":D="\u2696\uFE0F";break}let oe=M>0?`${D} ${M.toLocaleString()}`:"-",J=I!==y,ie=J?I:"";if(y=I,e){let ae=J?`${o.dim}${I}${o.reset}`:" ".repeat(I.length),de=H>0?`${o.dim}(~${H}t)${o.reset}`:"",ce=M>0?`${o.dim}(${D} ${M.toLocaleString()}t)${o.reset}`:"";n.push(`  ${o.dim}#${T.id}${o.reset}  ${ae}  ${v}  ${V} ${de} ${ce}`)}else n.push(`| #${T.id} | ${ie||"\u2033"} | ${v} | ${V} | ~${H} | ${oe} |`)}k&&n.push("")}let R=i[0],q=d[0];if(R&&(R.investigated||R.learned||R.completed||R.next_steps)&&(!q||R.created_at_epoch>q.created_at_epoch)&&(n.push(...F("Investigated",R.investigated,o.blue,e)),n.push(...F("Learned",R.learned,o.yellow,e)),n.push(...F("Completed",R.completed,o.green,e)),n.push(...F("Next Steps",R.next_steps,o.magenta,e))),b>0&&m>0){let l=Math.round(b/1e3);n.push(""),e?n.push(`${o.dim}\u{1F4B0} Access ${l}k tokens of past research & decisions for just ${g.toLocaleString()}t. Use claude-mem search to access memories by ID instead of re-reading files.${o.reset}`):n.push(`\u{1F4B0} Access ${l}k tokens of past research & decisions for just ${g.toLocaleString()}t. Use claude-mem search to access memories by ID instead of re-reading files.`)}}if(w.getConfig().enabled){let u=r.db.prepare(`
