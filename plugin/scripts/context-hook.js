#!/usr/bin/env node
import X from"path";import{homedir as Re}from"os";import{existsSync as Oe,readFileSync as Ne}from"fs";import{stdin as q}from"process";import Te from"better-sqlite3";import{join as f,dirname as _e,basename as Ue}from"path";import{homedir as Z}from"os";import{existsSync as Be,mkdirSync as ue}from"fs";import{fileURLToPath as me}from"url";function Ee(){return typeof __dirname<"u"?__dirname:_e(me(import.meta.url))}var je=Ee(),L=process.env.CLAUDE_MEM_DATA_DIR||f(Z(),".claude-mem"),W=process.env.CLAUDE_CONFIG_DIR||f(Z(),".claude"),We=f(L,"archives"),Ge=f(L,"logs"),Pe=f(L,"trash"),Ye=f(L,"backups"),Ke=f(L,"settings.json"),ee=f(L,"claude-mem.db"),qe=f(L,"vector-db"),Ve=f(W,"settings.json"),Je=f(W,"commands"),Qe=f(W,"CLAUDE.md");function se(d){ue(d,{recursive:!0})}var G=(a=>(a[a.DEBUG=0]="DEBUG",a[a.INFO=1]="INFO",a[a.WARN=2]="WARN",a[a.ERROR=3]="ERROR",a[a.SILENT=4]="SILENT",a))(G||{}),P=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=G[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,a){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),c=G[e].padEnd(5),_=s.padEnd(6),b="";r?.correlationId?b=`[${r.correlationId}] `:r?.sessionId&&(b=`[session-${r.sessionId}] `);let n="";a!=null&&(this.level===0&&typeof a=="object"?n=`
`+JSON.stringify(a,null,2):n=" "+this.formatData(a));let O="";if(r){let{sessionId:g,sdkSessionId:S,correlationId:m,...p}=r;Object.keys(p).length>0&&(O=` {${Object.entries(p).map(([B,x])=>`${B}=${x}`).join(", ")}}`)}let u=`[${i}] [${c}] [${_}] ${b}${t}${O}${n}`;e===3?console.error(u):console.log(u)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},A=new P;var U=class{db;constructor(){se(L),this.db=new Te(ee),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn(),this.removeToolUseIdUniqueConstraint()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)||(this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, 'active')
      `).run(e,e,s,i.toISOString(),c),console.error(`[SessionStore] Auto-created session record for session_id: ${e}`));let O=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, tool_use_id, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),r||null,a,t.tool_use_id||null,i.toISOString(),c);return{id:Number(O.lastInsertRowid),createdAtEpoch:c}}getObservationByToolUseId(e){return this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||null,a,i.toISOString(),c);return{id:Number(O.lastInsertRowid),createdAtEpoch:c}}markSessionCompleted(e){let s=new Date,t=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).run(s,t,r,e)}getEndlessModeStats(e){let t=this.db.prepare(`
      ...
      FROM sdk_sessions
      WHERE claude_session_id = ?
    `).get(e);return t?{originalTokens:t.endless_original_tokens||0,compressedTokens:t.endless_compressed_tokens||0,tokensSaved:t.endless_tokens_saved||0}:null}close(){this.db.close()}};import{existsSync as ge,readFileSync as he}from"fs";import{homedir as be}from"os";import Se from"path";function Y(d,e,s){if(d!==void 0){if(typeof d=="boolean")return d;if(typeof d=="string")return d.toLowerCase()==="true"}return e!==void 0?e.toLowerCase()==="true":s}function K(d,e,s){if(d!==void 0){if(typeof d=="number")return d;if(typeof d=="string"){let t=parseInt(d,10);if(!isNaN(t))return t}}if(e!==void 0){let t=parseInt(e,10);if(!isNaN(t))return t}return s}function fe(){let d=Se.join(be(),".claude-mem","settings.json"),e={};if(ge(d))try{e=JSON.parse(he(d,"utf-8"))}catch(b){A.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},b)}let s=Y(e.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),t=Y(e.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),r=K(e.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),a=K(e.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),i=K(e.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50),c=Y(e.env?.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,process.env.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,s),_={enabled:s,fallbackToOriginal:t,maxLookupTime:r,keepRecentToolUses:a,maxToolHistoryMB:i,enableSynchronousMode:c};return s?A.info("CONFIG","Endless Mode enabled",{fallback:t,maxLookupTime:`${r}ms`,keepRecent:a,maxToolHistoryMB:`${i}MB`,syncMode:c}):A.debug("CONFIG","Endless Mode disabled"),_}var w=class{static getConfig=fe;static clearCache(){}};function Ie(){try{let d=X.join(Re(),".claude","settings.json");if(Oe(d)){let e=JSON.parse(Ne(d,"utf-8"));if(e.env?.CLAUDE_MEM_CONTEXT_OBSERVATIONS){let s=parseInt(e.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10);if(!isNaN(s)&&s>0)return s}}}catch{}return parseInt(process.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS||"50",10)}var ve=Ie(),te=10,re=4,Le=1,o={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function ye(d){if(!d)return[];try{let e=JSON.parse(d);return Array.isArray(e)?e:[]}catch{return[]}}function Ae(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ke(d){return new Date(d).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Ce(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function De(d,e){return X.isAbsolute(d)?X.relative(e,d):d}function F(d,e,s,t){return e?t?[`${s}${d}:${o.reset} ${e}`,""]:[`**${d}**: ${e}`,""]:[]}async function ne(d,e=!1){let s=d?.cwd??process.cwd(),t=s?X.basename(s):"unknown-project",r=new U,a=r.db.prepare(`
    SELECT
