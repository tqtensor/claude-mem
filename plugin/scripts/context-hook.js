#!/usr/bin/env node
import B from"path";import{homedir as Re}from"os";import{existsSync as Ne,readFileSync as Oe}from"fs";import{stdin as q}from"process";import Te from"better-sqlite3";import{join as f,dirname as _e,basename as Ue}from"path";import{homedir as Z}from"os";import{existsSync as Xe,mkdirSync as ue}from"fs";import{fileURLToPath as me}from"url";function Ee(){return typeof __dirname<"u"?__dirname:_e(me(import.meta.url))}var je=Ee(),v=process.env.CLAUDE_MEM_DATA_DIR||f(Z(),".claude-mem"),G=process.env.CLAUDE_CONFIG_DIR||f(Z(),".claude"),Ge=f(v,"archives"),We=f(v,"logs"),Pe=f(v,"trash"),Ye=f(v,"backups"),Ke=f(v,"settings.json"),ee=f(v,"claude-mem.db"),qe=f(v,"vector-db"),Ve=f(G,"settings.json"),Je=f(G,"commands"),Qe=f(G,"CLAUDE.md");function se(d){ue(d,{recursive:!0})}var W=(a=>(a[a.DEBUG=0]="DEBUG",a[a.INFO=1]="INFO",a[a.WARN=2]="WARN",a[a.ERROR=3]="ERROR",a[a.SILENT=4]="SILENT",a))(W||{}),P=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=W[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,a){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),c=W[e].padEnd(5),_=s.padEnd(6),b="";r?.correlationId?b=`[${r.correlationId}] `:r?.sessionId&&(b=`[session-${r.sessionId}] `);let n="";a!=null&&(this.level===0&&typeof a=="object"?n=`
`+JSON.stringify(a,null,2):n=" "+this.formatData(a));let N="";if(r){let{sessionId:g,sdkSessionId:S,correlationId:m,...p}=r;Object.keys(p).length>0&&(N=` {${Object.entries(p).map(([X,M])=>`${X}=${M}`).join(", ")}}`)}let u=`[${i}] [${c}] [${_}] ${b}${t}${N}${n}`;e===3?console.error(u):console.log(u)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},A=new P;var U=class{db;constructor(){se(v),this.db=new Te(ee),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn()}initializeSchema(){try{this.db.exec(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,Le),i=r.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,te+ve);if(a.length===0&&i.length===0)return r.close(),e?`
