#!/usr/bin/env node
import{stdin as z}from"process";import{readFileSync as B,writeFileSync as Ae,renameSync as ve,copyFileSync as Ce}from"fs";import oe from"better-sqlite3";import{join as b,dirname as Z,basename as ee}from"path";import{homedir as P}from"os";import{existsSync as He,mkdirSync as se}from"fs";import{fileURLToPath as te}from"url";function re(){return typeof __dirname<"u"?__dirname:Z(te(import.meta.url))}var ne=re(),h=process.env.CLAUDE_MEM_DATA_DIR||b(P(),".claude-mem"),U=process.env.CLAUDE_CONFIG_DIR||b(P(),".claude"),Pe=b(h,"archives"),je=b(h,"logs"),$e=b(h,"trash"),I=b(h,"backups"),Ge=b(h,"settings.json"),j=b(h,"claude-mem.db"),We=b(h,"vector-db"),Ke=b(U,"settings.json"),Ye=b(U,"commands"),Je=b(U,"CLAUDE.md");function y(i){se(i,{recursive:!0})}function $(){return b(ne,"..","..")}function G(i){let e=new Date().toISOString().replace(/[:.]/g,"-").replace("T","_").slice(0,19),s=ee(i);return b(I,`${s}.backup.${e}`)}var w=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(w||{}),F=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=w[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,o){if(e<this.level)return;let n=new Date().toISOString().replace("T"," ").substring(0,23),a=w[e].padEnd(5),l=s.padEnd(6),p="";r?.correlationId?p=`[${r.correlationId}] `:r?.sessionId&&(p=`[session-${r.sessionId}] `);let g="";o!=null&&(this.level===0&&typeof o=="object"?g=`
`+JSON.stringify(o,null,2):g=" "+this.formatData(o));let _="";if(r){let{sessionId:m,sdkSessionId:O,correlationId:c,...d}=r;Object.keys(d).length>0&&(_=` {${Object.entries(d).map(([R,T])=>`${R}=${T}`).join(", ")}}`)}let E=`[${n}] [${a}] [${l}] ${p}${t}${_}${g}`;e===3?console.error(E):console.log(E)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},u=new F;var A=class{db;constructor(){y(h),this.db=new oe(j),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)||null}getFilesForSession(e){let t=this.db.prepare(`
      ...
      FROM observations
      WHERE sdk_session_id = ?
    `).all(e),r=new Set,o=new Set;for(let n of t){if(n.files_read)try{let a=JSON.parse(n.files_read);Array.isArray(a)&&a.forEach(l=>r.add(l))}catch{}if(n.files_modified)try{let a=JSON.parse(n.files_modified);Array.isArray(a)&&a.forEach(l=>o.add(l))}catch{}}return{filesRead:Array.from(r),filesModified:Array.from(o)}}getSessionById(e){return this.db.prepare(`
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,o=t==="date_asc"?"ASC":"DESC",n=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${n}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,o){let n=o?"AND project = ?":"",a=o?[o]:[],l,p;if(e!==null){let m=`
    ...
    `).run(s,t,r,e)}getEndlessModeStats(e){let t=this.db.prepare(`
      ...
      FROM sdk_sessions
      WHERE claude_session_id = ?
    `).get(e);return t?{originalTokens:t.endless_original_tokens||0,compressedTokens:t.endless_compressed_tokens||0,tokensSaved:t.endless_tokens_saved||0}:null}close(){this.db.close()}};function ie(i,e,s){return i==="PreCompact"?e?{continue:!0,suppressOutput:!0}:{continue:!1,stopReason:s.reason||"Pre-compact operation failed",suppressOutput:!0}:i==="SessionStart"?e&&s.context?{continue:!0,suppressOutput:!0,hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:s.context}}:{continue:!0,suppressOutput:!0}:i==="UserPromptSubmit"||i==="PostToolUse"?{continue:!0,suppressOutput:!0}:i==="Stop"?{continue:!0,suppressOutput:!0}:{continue:e,suppressOutput:!0,...s.reason&&!e?{stopReason:s.reason}:{}}}function N(i,e,s={}){let t=ie(i,e,s);return JSON.stringify(t)}import W from"path";import{homedir as ae}from"os";import{existsSync as K,readFileSync as de}from"fs";import{execSync as ce}from"child_process";var pe=100,le=500,ue=10;function D(){try{let i=W.join(ae(),".claude-mem","settings.json");if(K(i)){let e=JSON.parse(de(i,"utf-8")),s=parseInt(e.env?.CLAUDE_MEM_WORKER_PORT,10);if(!isNaN(s))return s}}catch{}return parseInt(process.env.CLAUDE_MEM_WORKER_PORT||"37777",10)}async function Y(){try{let i=D();return(await fetch(`http://127.0.0.1:${i}/health`,{signal:AbortSignal.timeout(pe)})).ok}catch{return!1}}async function _e(){try{let i=$(),e=W.join(i,"ecosystem.config.cjs");if(!K(e))throw new Error(`Ecosystem config not found at ${e}`);ce(`pm2 start "${e}"`,{cwd:i,stdio:"pipe",encoding:"utf-8"});for(let s=0;s<ue;s++)if(await new Promise(t=>setTimeout(t,le)),await Y())return!0;return!1}catch{return!1}}async function J(){if(await Y())return;if(!await _e()){let e=D();throw new Error(`Worker service failed to start on port ${e}.

Try manually running: pm2 start ecosystem.config.cjs
Or restart: pm2 restart claude-mem-worker`)}}import{existsSync as me,readFileSync as Ee}from"fs";import{homedir as Te}from"os";import ge from"path";var v=class{static config=null;static getConfig(){if(this.config)return this.config;let e=ge.join(Te(),".claude-mem","settings.json"),s={};if(me(e))try{s=JSON.parse(Ee(e,"utf-8"))}catch(p){u.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},p)}let t=this.getBooleanSetting(s.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),r=this.getBooleanSetting(s.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),o=this.getNumberSetting(s.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),n=this.getNumberSetting(s.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),a=this.getBooleanSetting(s.env?.CLAUDE_MEM_OBSERVE_EVERYTHING,process.env.CLAUDE_MEM_OBSERVE_EVERYTHING,t),l=this.getNumberSetting(s.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50);return this.config={enabled:t,fallbackToOriginal:r,maxLookupTime:o,keepRecentToolUses:n,observeEverything:a,maxToolHistoryMB:l},t?u.info("CONFIG","Endless Mode enabled",{fallback:r,maxLookupTime:`${o}ms`,keepRecent:n,observeEverything:a,maxToolHistoryMB:`${l}MB`}):u.debug("CONFIG","Endless Mode disabled"),this.config}static clearCache(){this.config=null}static getBooleanSetting(e,s,t){if(e!==void 0){if(typeof e=="boolean")return e;if(typeof e=="string")return e.toLowerCase()==="true"}return s!==void 0?s.toLowerCase()==="true":t}static getNumberSetting(e,s,t){if(e!==void 0){if(typeof e=="number")return e;if(typeof e=="string"){let r=parseInt(e,10);if(!isNaN(r))return r}}if(s!==void 0){let r=parseInt(s,10);if(!isNaN(r))return r}return t}};import{appendFileSync as fe}from"fs";import{homedir as be}from"os";import{join as Se}from"path";var Oe=Se(be(),".claude-mem","silent.log");function M(i,e,s=""){let t=new Date().toISOString(),a=((new Error().stack||"").split(`
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=a?`${a[1].split("/").pop()}:${a[2]}`:"unknown",p=`[${t}] [${l}] ${i}`;if(e!==void 0)try{p+=` ${JSON.stringify(e)}`}catch(g){p+=` [stringify error: ${g}]`}p+=`
`;try{fe(Oe,p)}catch(g){console.error("[silent-debug] Failed to write to log:",g)}return s}import{existsSync as Re,readFileSync as he,writeFileSync as Ne,appendFileSync as Ie,statSync as ye}from"fs";import{join as Le}from"path";var C=Le(I,"tool-outputs.jsonl");function q(i,e,s=Date.now()){y(I);let t=typeof e=="string"?e:JSON.stringify(e),r=Buffer.byteLength(t,"utf8"),n=JSON.stringify({tool_use_id:i,content:e,timestamp:s,size_bytes:r})+`
`;Ie(C,n,"utf8")}function V(i){if(!Re(C)||ye(C).size/(1024*1024)<=i)return;let r=he(C,"utf8").trim().split(`
`).filter(_=>_.length>0),o=[];for(let _ of r)try{o.push(JSON.parse(_))}catch{continue}o.sort((_,E)=>_.timestamp-E.timestamp);let n=i*1024*1024,a=0,l=0;for(let _=o.length-1;_>=0;_--){let E=o[_].size_bytes+100;if(a+E>n){l=_+1;break}a+=E}let g=o.slice(l).map(_=>JSON.stringify(_)).join(`
`)+`
`;Ne(C,g,"utf8")}var ke=new Set(["ListMcpResourcesTool","SlashCommand","Skill","TodoWrite","AskUserQuestion"]);function x(i,e){if(!i)return[];if(Array.isArray(i))return i;try{let s=JSON.parse(i);return Array.isArray(s)?s:[]}catch(s){return u.debug("HOOK",`Failed to parse ${e}`,{field:i,error:s}),[]}}function De(i){let e=[];e.push(`# ${i.title}`),i.subtitle&&e.push(`**${i.subtitle}**`),e.push(""),i.narrative&&(e.push(i.narrative),e.push(""));let s=x(i.facts,"facts");s.length>0&&(e.push("**Key Facts:**"),s.forEach(n=>e.push(`- ${n}`)),e.push(""));let t=x(i.concepts,"concepts");t.length>0&&(e.push(`**Concepts**: ${t.join(", ")}`),e.push(""));let r=x(i.files_read,"files_read");r.length>0&&(e.push(`**Files Read**: ${r.join(", ")}`),e.push(""));let o=x(i.files_modified,"files_modified");return o.length>0&&(e.push(`**Files Modified**: ${o.join(", ")}`),e.push("")),e.push("---"),e.push("*[Compressed by Endless Mode]*"),e.join(`
`)}async function Me(i,e,s){try{y(I);let c=G(i);Ce(i,c),u.info("HOOK","Created transcript backup",{original:i,backup:c})}catch(c){throw u.error("HOOK","Failed to create transcript backup",{transcriptPath:i},c),new Error("Backup creation failed - aborting transformation for safety")}let r=B(i,"utf-8").trim().split(`
`),o=!1,n=0,a=0,l=r.map((c,d)=>{if(!c.trim())return c;try{let S=JSON.parse(c);if(S.type==="user"){let T=S.message.content;if(Array.isArray(T))for(let f=0;f<T.length;f++){let L=T[f];if(L.type==="tool_result"){let k=L;if(k.tool_use_id===e){o=!0;try{q(e,k.content,Date.now()),u.debug("HOOK","Backed up original tool output",{toolUseId:e})}catch(Q){u.warn("HOOK","Failed to backup original tool output",{toolUseId:e},Q)}n=JSON.stringify(k.content).length;let X=De(s);a=X.length,k.content=X,u.success("HOOK","Transformed tool result",{toolUseId:e,originalSize:n,compressedSize:a,savings:`${Math.round((1-a/n)*100)}%`})}}}}return JSON.stringify(S)}catch(S){throw u.warn("HOOK","Malformed JSONL line in transcript",{lineIndex:d,error:S}),new Error(`Malformed JSONL line at index ${d}: ${S.message}`)}});if(!o)return u.warn("HOOK","Tool result not found in transcript",{toolUseId:e}),{originalTokens:0,compressedTokens:0};let p=`${i}.tmp`;Ae(p,l.join(`
`)+`
`,"utf-8");let _=B(p,"utf-8").trim().split(`
`);for(let c of _)c.trim()&&JSON.parse(c);ve(p,i);let E=4,m=Math.ceil(n/E),O=Math.ceil(a/E);u.success("HOOK","Transcript transformation complete",{toolUseId:e,originalSize:n,compressedSize:a,savings:`${Math.round((1-a/n)*100)}%`});try{let c=v.getConfig();c.maxToolHistoryMB>0&&(V(c.maxToolHistoryMB),u.debug("HOOK","Trimmed tool output backup",{maxSizeMB:c.maxToolHistoryMB}))}catch(c){u.warn("HOOK","Failed to trim tool output backup",{},c)}return{originalTokens:m,compressedTokens:O}}async function xe(i){if(!i){u.warn("HOOK","PostToolUse called with no input"),console.log(N("PostToolUse",!0));return}let{session_id:e,cwd:s,tool_name:t,tool_input:r,tool_response:o,transcript_path:n,tool_use_id:a}=i;if(ke.has(t)){console.log(N("PostToolUse",!0));return}await J();let l=new A,p=l.createSDKSession(e,"",""),g=l.getPromptCounter(p);l.close();let _=u.formatTool(t,r),E=D(),m=a;if(!m&&n)try{let S=B(n,"utf-8").trim().split(`
`);for(let R=S.length-1;R>=0;R--){let T=JSON.parse(S[R]);if(T.type==="user"&&Array.isArray(T.message.content)){for(let f of T.message.content)if(f.type==="tool_result"&&f.tool_use_id){m=f.tool_use_id;break}if(m)break}}}catch(d){M("Failed to extract tool_use_id from transcript",{error:d})}u.dataIn("HOOK",`PostToolUse: ${_}`,{sessionId:p,workerPort:E,toolUseId:m||M("tool_use_id not found in transcript",{toolName:t},"(none)")});let O=v.getConfig(),c=O.enabled&&m&&n;M("Endless Mode Check",{configEnabled:O.enabled,hasToolUseId:!!m,hasTranscriptPath:!!n,isEndlessModeEnabled:c,toolName:t,toolUseId:m,allInputKeys:Object.keys(i).join(", ")});try{let d=c?`http://127.0.0.1:${E}/sessions/${p}/observations?wait_until_obs_is_saved=true`:`http://127.0.0.1:${E}/sessions/${p}/observations`,S=c?9e4:2e3,R=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool_name:t,tool_input:r!==void 0?JSON.stringify(r):"{}",tool_response:o!==void 0?JSON.stringify(o):"{}",prompt_number:g,cwd:s||"",tool_use_id:m}),signal:AbortSignal.timeout(S)});if(!R.ok){let f=await R.text();u.failure("HOOK","Failed to send observation",{sessionId:p,status:R.status},f),console.log(N("PostToolUse",!0));return}let T=await R.json();if(c)if(T.status==="completed"&&T.observation){u.success("HOOK","Observation ready, transforming transcript",{sessionId:p,toolUseId:m,processingTimeMs:T.processing_time_ms});try{let f=await Me(n,m,T.observation);if(f.originalTokens>0){let L=new A;L.incrementEndlessModeStats(e,f.originalTokens,f.compressedTokens),L.close()}}catch(f){u.failure("HOOK","Transcript transformation failed",{sessionId:p,toolUseId:m},f)}}else T.status==="timeout"?u.warn("HOOK","Endless Mode timeout - using full output",{sessionId:p,toolUseId:m,processingTimeMs:T.processing_time_ms,message:T.message}):u.debug("HOOK","Endless Mode received non-standard response - continuing",{sessionId:p,toolUseId:m,status:T.status||"unknown"});else u.debug("HOOK","Observation sent successfully (async mode)",{sessionId:p,toolName:t})}catch(d){if(d.cause?.code==="ECONNREFUSED"){u.failure("HOOK","Worker connection refused",{sessionId:p},d),console.log(N("PostToolUse",!0,"Worker connection failed. Try: pm2 restart claude-mem-worker"));return}if(d.name==="TimeoutError"||d.message?.includes("timed out")){u.warn("HOOK","Observation request timed out - continuing",{sessionId:p,toolName:t}),console.log(N("PostToolUse",!0));return}u.warn("HOOK","Observation request failed - continuing anyway",{sessionId:p,toolName:t,error:d.message}),console.log(N("PostToolUse",!0));return}console.log(N("PostToolUse",!0))}var H="";z.on("data",i=>H+=i);z.on("end",async()=>{let i=H?JSON.parse(H):void 0;await xe(i)});
