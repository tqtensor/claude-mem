...
`;Je(B,i,"utf8")}function ce(r){if(!We(B)||Ye(B).size/(1024*1024)<=r)return;let o=Ke(B,"utf8").trim().split(`
...
`)+`
`;Ge(B,T,"utf8")}var pe=100;function qe(r){let e=(r.match(/<private>/g)||[]).length,s=(r.match(/<claude-mem-context>/g)||[]).length;return e+s}function Q(r){if(typeof r!="string")return U("[tag-stripping] received non-string for JSON context:",{type:typeof r}),"{}";let e=qe(r);return e>pe&&U("[tag-stripping] tag count exceeds limit, truncating:",{tagCount:e,maxAllowed:pe,contentLength:r.length}),r.replace(/<claude-mem-context>[\s\S]*?<\/claude-mem-context>/g,"").replace(/<private>[\s\S]*?<\/private>/g,"").trim()}import{readFileSync as Z,writeFileSync as ue,renameSync as Ve,copyFileSync as Qe}from"fs";import{randomUUID as de}from"crypto";async function me(r,e){try{x(M);let p=j(r);Qe(r,p),d.debug("HOOK","Created transcript backup before clearing input",{original:r,backup:p})}catch(p){throw d.error("HOOK","Failed to create transcript backup",{transcriptPath:r},p),new Error("Backup creation failed - aborting transformation for safety")}let t=Z(r,"utf-8").trim().split(`
...
`)+`
`,"utf-8");let c=Z(i,"utf-8").trim().split(`
`);for(let p of c)p.trim()&&JSON.parse(p);return Ve(i,r),o}async function _e(r,e,s,t){if(t.length===0){d.debug("HOOK","No observations to inject");return}let o=Z(r,"utf-8"),n=o.trim().split(`
`),i=JSON.parse(n[n.length-1]),a={isSidechain:!1,userType:i.userType||"user",cwd:s,sessionId:e,version:i.version||"1.0",timestamp:new Date().toISOString()},c=`toolu_mem_${Date.now()}`,p={...a,type:"assistant",uuid:de(),message:{id:`msg_${Date.now()}`,type:"message",role:"assistant",model:"claude-3-5-sonnet-20241022",content:[{type:"tool_use",id:c,name:"claude-mem-fetch-observations",input:{observation_ids:t.map(S=>S.id),note:"Fetching compressed observations for context"}}],stop_reason:"tool_use"}},T=t.map(S=>Ze(S)).join(`

---

`),m={...a,type:"user",uuid:de(),message:{role:"user",content:[{type:"tool_result",tool_use_id:c,content:T}]}},f=[JSON.stringify(p),JSON.stringify(m)];ue(r,o+`
`+f.join(`
`)+`
