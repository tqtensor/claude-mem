#!/usr/bin/env node
import V from"better-sqlite3";import{join as p,dirname as W,basename as pe}from"path";import{homedir as D}from"os";import{existsSync as me,mkdirSync as $}from"fs";import{fileURLToPath as B}from"url";function K(){return typeof __dirname<"u"?__dirname:W(B(import.meta.url))}var Y=K(),u=process.env.CLAUDE_MEM_DATA_DIR||p(D(),".claude-mem"),R=process.env.CLAUDE_CONFIG_DIR||p(D(),".claude"),_e=p(u,"archives"),Te=p(u,"logs"),ge=p(u,"trash"),Se=p(u,"backups"),be=p(u,"settings.json"),w=p(u,"claude-mem.db"),he=p(R,"settings.json"),fe=p(R,"commands"),Ne=p(R,"CLAUDE.md");function U(n){$(n,{recursive:!0})}function M(){return p(Y,"..","..")}var I=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(I||{}),O=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=I[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,t){return`obs-${e}-${t}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let T="";if(r){let{sessionId:N,sdkSessionId:ne,correlationId:ie,...y}=r;Object.keys(y).length>0&&(T=` {${Object.entries(y).map(([j,G])=>`${j}=${G}`).join(", ")}}`)}let m=`[${d}] [${a}] [${i}] ${c}${s}${T}${l}`;e===3?console.error(m):console.log(m)}debug(e,t,s,r){this.log(0,e,t,s,r)}info(e,t,s,r){this.log(1,e,t,s,r)}warn(e,t,s,r){this.log(2,e,t,s,r)}error(e,t,s,r){this.log(3,e,t,s,r)}dataIn(e,t,s,r){this.info(e,`\u2192 ${t}`,s,r)}dataOut(e,t,s,r){this.info(e,`\u2190 ${t}`,s,r)}success(e,t,s,r){this.info(e,`\u2713 ${t}`,s,r)}failure(e,t,s,r){this.error(e,`\u2717 ${t}`,s,r)}timing(e,t,s,r){this.info(e,`\u23F1 ${t}`,r,{duration:`${s}ms`})}},E=new O;var S=class{db;constructor(){U(u),this.db=new V(w),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable()}initializeSchema(){try{this.db.exec(`
    ...
    `).run(t.toISOString(),s,e)}cleanupOrphanedSessions(){let e=new Date,t=e.getTime();return this.db.prepare(`
      ...
      SET status = 'failed', completed_at = ?, completed_at_epoch = ?
      WHERE status = 'active'
    `).run(e.toISOString(),t).changes}close(){this.db.close()}};function q(n,e,t){return n==="PreCompact"?e?{continue:!0,suppressOutput:!0}:{continue:!1,stopReason:t.reason||"Pre-compact operation failed",suppressOutput:!0}:n==="SessionStart"?e&&t.context?{continue:!0,suppressOutput:!0,hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:t.context}}:{continue:!0,suppressOutput:!0}:n==="UserPromptSubmit"||n==="PostToolUse"?{continue:!0,suppressOutput:!0}:n==="Stop"?{continue:!0,suppressOutput:!0}:{continue:e,suppressOutput:!0,...t.reason&&!e?{stopReason:t.reason}:{}}}function b(n,e,t={}){let s=q(n,e,t);return JSON.stringify(s)}import k from"path";import{existsSync as C}from"fs";import{spawn as se}from"child_process";import{readFileSync as J,writeFileSync as Q,existsSync as v,mkdirSync as z}from"fs";import{join as Z,dirname as ee}from"path";import{homedir as te}from"os";var _={model:"claude-sonnet-4-5",workerPort:37777,enableMemoryStorage:!0,enableContextInjection:!0,contextDepth:5},g={model:"AI model to use for processing observations and generating summaries",workerPort:"Port for the background worker service HTTP API",enableMemoryStorage:"Enable/disable saving tool observations to the database",enableContextInjection:"Enable/disable context injection at session start",contextDepth:"Number of recent sessions to load when injecting context (higher = more history, more tokens)"},h=["claude-haiku-4-5","claude-sonnet-4-5","claude-opus-4","claude-3-7-sonnet"],A=class{settingsPath;cachedSettings=null;constructor(e){this.settingsPath=e||Z(te(),".claude-mem","settings.json")}loadSettings(){if(this.cachedSettings)return this.cachedSettings;let e={};if(v(this.settingsPath))try{let s=J(this.settingsPath,"utf-8");e=JSON.parse(s)}catch(s){console.error(`[claude-mem] Failed to parse settings file: ${s.message}`)}let t={model:e.model||_.model,workerPort:e.workerPort||_.workerPort,enableMemoryStorage:e.enableMemoryStorage??_.enableMemoryStorage,enableContextInjection:e.enableContextInjection??_.enableContextInjection,contextDepth:e.contextDepth||_.contextDepth};return this.validateSettings(t),this.cachedSettings=t,t}validateSettings(e){if(!h.includes(e.model))throw new Error(`Invalid model: ${e.model}. Must be one of: ${h.join(", ")}`);if(typeof e.workerPort!="number"||e.workerPort<1||e.workerPort>65535)throw new Error(`Invalid workerPort: ${e.workerPort}. Must be between 1-65535`);if(typeof e.enableMemoryStorage!="boolean")throw new Error(`Invalid enableMemoryStorage: ${e.enableMemoryStorage}. Must be boolean`);if(typeof e.enableContextInjection!="boolean")throw new Error(`Invalid enableContextInjection: ${e.enableContextInjection}. Must be boolean`);if(typeof e.contextDepth!="number"||e.contextDepth<1||e.contextDepth>50)throw new Error(`Invalid contextDepth: ${e.contextDepth}. Must be between 1-50`)}get(){return this.loadSettings()}getWithDescriptions(){let e=this.get();return{model:{value:e.model,description:g.model,options:h},workerPort:{value:e.workerPort,description:g.workerPort},enableMemoryStorage:{value:e.enableMemoryStorage,description:g.enableMemoryStorage},enableContextInjection:{value:e.enableContextInjection,description:g.enableContextInjection},contextDepth:{value:e.contextDepth,description:g.contextDepth}}}set(e){let s={...this.get(),...e};this.validateSettings(s);let r=ee(this.settingsPath);v(r)||z(r,{recursive:!0});try{Q(this.settingsPath,JSON.stringify(s,null,2),"utf-8"),this.cachedSettings=s}catch(o){throw new Error(`Failed to save settings: ${o.message}`)}}reset(){this.set(_)}getDefaults(){return{..._}}getModelOptions(){return[...h]}exists(){return v(this.settingsPath)}getPath(){return this.settingsPath}},L=null;function f(){return L||(L=new A),L}function re(){return`http://127.0.0.1:${f().get().workerPort}/health`}async function P(){try{return(await fetch(re(),{signal:AbortSignal.timeout(500)})).ok}catch{return!1}}async function X(){try{if(await P())return!0;console.error("[claude-mem] Worker not responding, starting...");let n=M(),e=k.join(n,"plugin","scripts","worker-service.cjs");if(!C(e))return console.error(`[claude-mem] Worker service not found at ${e}`),!1;let t=k.join(n,"ecosystem.config.cjs"),s=k.join(n,"node_modules",".bin","pm2");if(!C(s))throw new Error(`PM2 binary not found at ${s}. This is a bundled dependency - try running: npm install`);if(!C(t))throw new Error(`PM2 ecosystem config not found at ${t}. Plugin installation may be corrupted.`);let r=se(s,["start",t],{detached:!0,stdio:"ignore",cwd:n});r.on("error",o=>{throw new Error(`Failed to spawn PM2: ${o.message}`)}),r.unref(),console.error("[claude-mem] Worker started with PM2");for(let o=0;o<3;o++)if(await new Promise(d=>setTimeout(d,500)),await P())return console.error("[claude-mem] Worker is healthy"),!0;return console.error("[claude-mem] Worker failed to become healthy after startup"),!1}catch(n){return console.error(`[claude-mem] Failed to start worker: ${n.message}`),!1}}var oe=new Set(["ListMcpResourcesTool"]);async function F(n){if(!n)throw new Error("saveHook requires input");let e=f().get();if(!e.enableMemoryStorage){console.log(b("PostToolUse",!0));return}let{session_id:t,tool_name:s,tool_input:r,tool_output:o}=n;if(oe.has(s)){console.log(b("PostToolUse",!0));return}if(!await X())throw new Error("Worker service failed to start or become healthy");let a=new S,i=a.createSDKSession(t,"",""),c=a.getPromptCounter(i);a.close();let l=E.formatTool(s,r),T=e.workerPort;E.dataIn("HOOK",`PostToolUse: ${l}`,{sessionId:i,workerPort:T});let m=await fetch(`http://127.0.0.1:${T}/sessions/${i}/observations`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool_name:s,tool_input:r!==void 0?JSON.stringify(r):"{}",tool_output:o!==void 0?JSON.stringify(o):"{}",prompt_number:c}),signal:AbortSignal.timeout(2e3)});if(!m.ok){let N=await m.text();throw E.failure("HOOK","Failed to send observation",{sessionId:i,status:m.status},N),new Error(`Failed to send observation to worker: ${m.status} ${N}`)}E.debug("HOOK","Observation sent successfully",{sessionId:i,toolName:s}),console.log(b("PostToolUse",!0))}import{stdin as H}from"process";var x="";H.on("data",n=>x+=n);H.on("end",async()=>{let n=x.trim()?JSON.parse(x):void 0;await F(n),process.exit(0)});
