#!/usr/bin/env node
import ze from"path";import{stdin as ie}from"process";import ge from"better-sqlite3";import{join as b,dirname as ae,basename as ce}from"path";import{homedir as J}from"os";import{existsSync as ss,mkdirSync as de}from"fs";import{fileURLToPath as pe}from"url";function le(){return typeof __dirname<"u"?__dirname:ae(pe(import.meta.url))}var ue=le(),I=process.env.CLAUDE_MEM_DATA_DIR||b(J(),".claude-mem"),H=process.env.CLAUDE_CONFIG_DIR||b(J(),".claude"),rs=b(I,"archives"),os=b(I,"logs"),ns=b(I,"trash"),D=b(I,"backups"),is=b(I,"settings.json"),z=b(I,"claude-mem.db"),as=b(I,"vector-db"),cs=b(H,"settings.json"),ds=b(H,"commands"),ps=b(H,"CLAUDE.md");function M(o){de(o,{recursive:!0})}function q(){return b(ue,"..","..")}function V(o){let e=new Date().toISOString().replace(/[:.]/g,"-").replace("T","_").slice(0,19),s=ce(o);return b(D,`${s}.backup.${e}`)}var P=(n=>(n[n.DEBUG=0]="DEBUG",n[n.INFO=1]="INFO",n[n.WARN=2]="WARN",n[n.ERROR=3]="ERROR",n[n.SILENT=4]="SILENT",n))(P||{}),X=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=P[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,n){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),a=P[e].padEnd(5),d=s.padEnd(6),u="";r?.correlationId?u=`[${r.correlationId}] `:r?.sessionId&&(u=`[session-${r.sessionId}] `);let S="";n!=null&&(this.level===0&&typeof n=="object"?S=`
`+JSON.stringify(n,null,2):S=" "+this.formatData(n));let T="";if(r){let{sessionId:p,sdkSessionId:R,correlationId:_,...c}=r;Object.keys(c).length>0&&(T=` {${Object.entries(c).map(([f,g])=>`${f}=${g}`).join(", ")}}`)}let m=`[${i}] [${a}] [${d}] ${u}${t}${T}${S}`;e===3?console.error(m):console.log(m)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},l=new X;import{appendFileSync as me}from"fs";import{homedir as _e}from"os";import{join as Ee}from"path";var Te=Ee(_e(),".pm2","logs","claude-mem-worker-error.log");function E(o,e,s=""){let t=new Date().toISOString(),a=((new Error().stack?.split(`
...
`;try{me(Te,u)}catch{}return s}var C=class{db;constructor(){M(I),this.db=new ge(z),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn(),this.removeToolUseIdUniqueConstraint()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)||(this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, 'active')
      `).run(e,e,s,i.toISOString(),a),console.error(`[SessionStore] Auto-created session record for session_id: ${e}`));let T=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, tool_use_id, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),r||E("SessionStore.storeObservation: promptNumber is null",{sdkSessionId:e},null),n,t.tool_use_id||E("SessionStore.storeObservation: tool_use_id is null",{sdkSessionId:e},null),i.toISOString(),a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}getObservationByToolUseId(e){return this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||E("SessionStore.storeSummary: promptNumber is null",{sdkSessionId:e},null),n,i.toISOString(),a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}markSessionCompleted(e){let s=new Date,t=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,n=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${i}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,n){let i=n?"AND project = ?":"",a=n?[n]:[],d,u;if(e!==null){let p=`
    ...
    `).run(s,t,r,e)}getEndlessModeStats(e){let t=this.db.prepare(`
      ...
      FROM sdk_sessions
      WHERE claude_session_id = ?
    `).get(e);return t?{originalTokens:t.endless_original_tokens||E("SessionStore.getEndlessModeStats: endless_original_tokens is null",{claudeSessionId:e},0),compressedTokens:t.endless_compressed_tokens||E("SessionStore.getEndlessModeStats: endless_compressed_tokens is null",{claudeSessionId:e},0),tokensSaved:t.endless_tokens_saved||E("SessionStore.getEndlessModeStats: endless_tokens_saved is null",{claudeSessionId:e},0)}:null}close(){this.db.close()}};function Se(o,e,s){return o==="PreCompact"?e?{continue:!0,suppressOutput:!0}:{continue:!1,stopReason:s.reason||E("hook-response: options.reason is null",{},"Pre-compact operation failed"),suppressOutput:!0}:o==="SessionStart"?e&&s.context?{continue:!0,suppressOutput:!0,hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:s.context}}:{continue:!0,suppressOutput:!0}:o==="UserPromptSubmit"||o==="PostToolUse"?{continue:!0,suppressOutput:!0}:o==="Stop"?{continue:!0,suppressOutput:!0}:{continue:e,suppressOutput:!0,...s.reason&&!e?{stopReason:s.reason}:{}}}function N(o,e,s={}){let t=Se(o,e,s);return JSON.stringify(t)}import Q from"path";import{homedir as fe}from"os";import{existsSync as Z,readFileSync as be}from"fs";import{execSync as Oe}from"child_process";var he=100,Re=500,Ne=10;function U(){try{let o=Q.join(fe(),".claude-mem","settings.json");if(Z(o)){let e=JSON.parse(be(o,"utf-8")),s=parseInt(e.env?.CLAUDE_MEM_WORKER_PORT,10);if(!isNaN(s))return s}}catch{}return parseInt(process.env.CLAUDE_MEM_WORKER_PORT||"37777",10)}async function ee(){try{let o=U();return(await fetch(`http://127.0.0.1:${o}/health`,{signal:AbortSignal.timeout(he)})).ok}catch{return!1}}async function ye(){try{let o=q(),e=Q.join(o,"ecosystem.config.cjs");if(!Z(e))throw new Error(`Ecosystem config not found at ${e}`);Oe(`pm2 start "${e}"`,{cwd:o,stdio:"pipe",encoding:"utf-8"});for(let s=0;s<Ne;s++)if(await new Promise(t=>setTimeout(t,Re)),await ee())return!0;return!1}catch{return!1}}async function w(){if(await ee())return;if(!await ye()){let e=U();throw new Error(`Worker service failed to start on port ${e}.

Try manually running: pm2 start ecosystem.config.cjs
Or restart: pm2 restart claude-mem-worker`)}}import{stdin as oe}from"process";import{readFileSync as G,writeFileSync as Fe,renameSync as Be,copyFileSync as He,existsSync as Pe,createReadStream as Xe}from"fs";import{dirname as $e,join as je,basename as We}from"path";import{createInterface as Ge}from"readline";import{existsSync as Ie,readFileSync as ve}from"fs";import{homedir as Ae}from"os";import Le from"path";function $(o,e,s){if(o!==void 0){if(typeof o=="boolean")return o;if(typeof o=="string")return o.toLowerCase()==="true"}return e!==void 0?e.toLowerCase()==="true":s}function j(o,e,s){if(o!==void 0){if(typeof o=="number")return o;if(typeof o=="string"){let t=parseInt(o,10);if(!isNaN(t))return t}}if(e!==void 0){let t=parseInt(e,10);if(!isNaN(t))return t}return s}function Ce(){let o=Le.join(Ae(),".claude-mem","settings.json"),e={};if(Ie(o))try{e=JSON.parse(ve(o,"utf-8"))}catch(u){l.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},u)}let s=$(e.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),t=$(e.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),r=j(e.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),n=j(e.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),i=j(e.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50),a=$(e.env?.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,process.env.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,s),d={enabled:s,fallbackToOriginal:t,maxLookupTime:r,keepRecentToolUses:n,maxToolHistoryMB:i,enableSynchronousMode:a};return s?l.info("CONFIG","Endless Mode enabled",{fallback:t,maxLookupTime:`${r}ms`,keepRecent:n,maxToolHistoryMB:`${i}MB`,syncMode:a}):l.debug("CONFIG","Endless Mode disabled"),d}var k=class{static getConfig=Ce;static clearCache(){}};import{existsSync as ke,readFileSync as De,writeFileSync as Me,appendFileSync as Ue,statSync as xe}from"fs";import{join as we}from"path";var x=we(D,"tool-outputs.jsonl");function W(o,e,s=Date.now()){M(D);let t=typeof e=="string"?e:JSON.stringify(e),r=Buffer.byteLength(t,"utf8"),i=JSON.stringify({tool_use_id:o,content:e,timestamp:s,size_bytes:r})+`
`;Ue(x,i,"utf8")}function se(o){if(!ke(x)||xe(x).size/(1024*1024)<=o)return;let r=De(x,"utf8").trim().split(`
`).filter(T=>T.length>0),n=[];for(let T of r)try{n.push(JSON.parse(T))}catch{continue}n.sort((T,m)=>T.timestamp-m.timestamp);let i=o*1024*1024,a=0,d=0;for(let T=n.length-1;T>=0;T--){let m=n[T].size_bytes+100;if(a+m>i){d=T+1;break}a+=m}let S=n.slice(d).map(T=>JSON.stringify(T)).join(`
`)+`
`;Me(x,S,"utf8")}var Ke=new Set(["ListMcpResourcesTool","SlashCommand","Skill","TodoWrite","AskUserQuestion"]);function F(o,e){if(!o)return[];if(Array.isArray(o))return o;try{let s=JSON.parse(o);return Array.isArray(s)?s:[]}catch(s){return l.debug("HOOK",`Failed to parse ${e}`,{field:o,error:s}),[]}}function te(o){let e=[];e.push(o.title),o.subtitle&&e.push(o.subtitle),o.narrative&&e.push(o.narrative);let s=F(o.facts,"facts");s.length>0&&e.push(`Facts: ${s.join("; ")}`);let t=F(o.concepts,"concepts");t.length>0&&e.push(`Concepts: ${t.join(", ")}`);let r=F(o.files_read,"files_read");r.length>0&&e.push(`Files read: ${r.join(", ")}`);let n=F(o.files_modified,"files_modified");return n.length>0&&e.push(`Files modified: ${n.join(", ")}`),e.join(". ")}async function Ye(o){let e=new Set;try{let s=Xe(o),t=Ge({input:s,crlfDelay:1/0});for await(let i of t)if(i.includes("agentId"))try{let a=JSON.parse(i);a.toolUseResult?.agentId&&e.add(a.toolUseResult.agentId)}catch{}let r=$e(o),n=Array.from(e).map(i=>je(r,`agent-${i}.jsonl`)).filter(i=>Pe(i));return l.debug("HOOK","Discovered agent transcripts",{agentCount:e.size,filesFound:n.length,agentFiles:n.map(i=>We(i))}),n}catch(s){return l.warn("HOOK","Failed to discover agent files",{mainTranscriptPath:o},s),[]}}async function ne(o,e){let s=await Ye(o),t=await re(o,e),r=0,n=0;for(let i of s)try{let a=await re(i,e);r+=a.originalTokens,n+=a.compressedTokens}catch(a){l.warn("HOOK","Failed to transform agent transcript",{agentFile:i},a)}return{originalTokens:t.originalTokens+r,compressedTokens:t.compressedTokens+n}}async function re(o,e){try{M(D);let p=V(o);He(o,p),l.info("HOOK","Created transcript backup",{original:o,backup:p})}catch(p){throw l.error("HOOK","Failed to create transcript backup",{transcriptPath:o},p),new Error("Backup creation failed - aborting transformation for safety")}let t=G(o,"utf-8").trim().split(`
`),r={totalOriginalSize:0,totalCompressedSize:0,transformCount:0},n=new C,i=t.map((p,R)=>{if(!p.trim())return p;try{let _=JSON.parse(p);if(!Array.isArray(_.message?.content))return p;let c=!1;return _.type==="assistant"&&_.message.content.forEach(f=>{if(f.type==="tool_use"){let g=f;if(!g.id)return;let h=n.getAllObservationsForToolUseId(g.id);if(h.length>0){let v=JSON.stringify(g.input).length,A=h.map(y=>te(y)).join(" | "),L=A.length;if(L<v){try{W(g.id,JSON.stringify(g.input),Date.now())}catch(y){l.warn("HOOK","Failed to backup original tool input",{toolUseId:g.id},y)}g.input={_compressed:A},r.totalOriginalSize+=v,r.totalCompressedSize+=L,r.transformCount++,c=!0,l.success("HOOK","Transformed tool_use input",{toolUseId:g.id,originalSize:v,compressedSize:L,savings:`${Math.round((1-L/v)*100)}%`})}else l.debug("HOOK","Skipped input transformation (observation not shorter)",{toolUseId:g.id,originalSize:v,compressedSize:L})}}}),_.type==="user"&&_.message.content.forEach(f=>{if(f.type==="tool_result"){let g=f,h=g.tool_use_id;if(!h)return;let v=n.getAllObservationsForToolUseId(h);if(v.length>0){let A=JSON.stringify(g.content).length,L=v.map(B=>te(B)).join(" | "),y=L.length;if(y<A){try{W(h,JSON.stringify(g.content),Date.now())}catch(B){l.warn("HOOK","Failed to backup original tool output",{toolUseId:h},B)}g.content=L,r.totalOriginalSize+=A,r.totalCompressedSize+=y,r.transformCount++,c=!0,l.success("HOOK","Transformed tool_result output",{toolUseId:h,originalSize:A,compressedSize:y,savings:`${Math.round((1-y/A)*100)}%`})}else l.debug("HOOK","Skipped output transformation (observation not shorter)",{toolUseId:h,originalSize:A,compressedSize:y})}}}),c?JSON.stringify(_):p}catch(_){throw l.warn("HOOK","Malformed JSONL line in transcript",{lineIndex:R,error:_}),new Error(`Malformed JSONL line at index ${R}: ${_.message}`)}});n.close();let a=`${o}.tmp`;Fe(a,i.join(`
`)+`
`,"utf-8");let u=G(a,"utf-8").trim().split(`
`);for(let p of u)p.trim()&&JSON.parse(p);Be(a,o);let S=4,T=Math.ceil(r.totalOriginalSize/S),m=Math.ceil(r.totalCompressedSize/S);l.success("HOOK","Transcript transformation complete",{toolUseId:e,transformCount:r.transformCount,totalOriginalSize:r.totalOriginalSize,totalCompressedSize:r.totalCompressedSize,savings:r.totalOriginalSize>0?`${Math.round((1-r.totalCompressedSize/r.totalOriginalSize)*100)}%`:"0%"});try{let p=k.getConfig();p.maxToolHistoryMB>0&&(se(p.maxToolHistoryMB),l.debug("HOOK","Trimmed tool output backup",{maxSizeMB:p.maxToolHistoryMB}))}catch(p){l.warn("HOOK","Failed to trim tool output backup",{},p)}return{originalTokens:T,compressedTokens:m}}async function Je(o){o||(l.warn("HOOK","PostToolUse called with no input"),console.log(N("PostToolUse",!0)),process.exit(0));let{session_id:e,cwd:s,tool_name:t,tool_input:r,tool_response:n,transcript_path:i,tool_use_id:a}=o;Ke.has(t)&&(console.log(N("PostToolUse",!0)),process.exit(0)),await w();let d=new C,u=d.createSDKSession(e,"",""),S=d.getPromptCounter(u);d.close();let T=l.formatTool(t,r),m=U(),p=a;if(!p&&i)try{let O=G(i,"utf-8").trim().split(`
`);for(let f=O.length-1;f>=0;f--){let g=JSON.parse(O[f]);if(g.type==="user"&&Array.isArray(g.message.content)){for(let h of g.message.content)if(h.type==="tool_result"&&h.tool_use_id){p=h.tool_use_id;break}if(p)break}}}catch(c){E("Failed to extract tool_use_id from transcript",{error:c})}l.dataIn("HOOK",`PostToolUse: ${T}`,{sessionDbId:u,claudeSessionId:e,workerPort:m,toolUseId:p||E("tool_use_id not found in transcript",{toolName:t},"(none)")});let R=k.getConfig(),_=!!(R.enabled&&p&&i);E("Endless Mode Check",{configEnabled:R.enabled,hasToolUseId:!!p,hasTranscriptPath:!!i,isEndlessModeEnabled:_,toolName:t,toolUseId:p,allInputKeys:Object.keys(o).join(", ")});try{let c=_?parseInt(process.env.CLAUDE_MEM_ENDLESS_WAIT_TIMEOUT_MS||(E("CLAUDE_MEM_ENDLESS_WAIT_TIMEOUT_MS not set, using default 90000ms"),"90000"),10):2e3,O=await fetch(`http://127.0.0.1:${m}/sessions/${u}/observations?wait_until_obs_is_saved=${_}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tool_name:t,tool_input:r!==void 0?JSON.stringify(r):"{}",tool_response:n!==void 0?JSON.stringify(n):"{}",prompt_number:S,cwd:s||E("save-hook: cwd missing",{sessionDbId:u,tool_name:t}),tool_use_id:p,transcript_path:i||E("save-hook: transcript_path missing",{sessionDbId:u,tool_name:t})}),signal:AbortSignal.timeout(c)});if(!O.ok){let g=await O.text();l.failure("HOOK","Failed to send observation",{sessionDbId:u,status:O.status},g),console.log(N("PostToolUse",!0)),process.exit(0)}let f=await O.json();f.status==="completed"?console.log("[save-hook] \u2705 Observation created, transcript transformed"):f.status==="skipped"?console.log("[save-hook] \u23ED\uFE0F  No observation needed, continuing"):f.status==="timeout"&&console.warn(`[save-hook] \u23F1\uFE0F  Timeout after ${c}ms - processing async`),l.debug("HOOK","Observation sent successfully",{sessionDbId:u,toolName:t,mode:_?"synchronous (Endless Mode)":"async"})}catch(c){if(c.cause?.code==="ECONNREFUSED"){let O="Worker connection failed. Try: pm2 restart claude-mem-worker";l.failure("HOOK","Worker connection refused",{sessionDbId:u},c),console.error(`[save-hook] ${O}`),console.log(N("PostToolUse",!1,{reason:O})),process.exit(2)}console.warn("[save-hook] \u274C Failed to send observation:",c.message),l.warn("HOOK","Observation request failed - continuing anyway",{sessionDbId:u,toolName:t,error:c.message}),console.log(N("PostToolUse",!0)),process.exit(0)}console.log(N("PostToolUse",!0)),process.exit(0)}var K="";oe.on("data",o=>K+=o);oe.on("end",async()=>{try{let o=K?JSON.parse(K):void 0;await Je(o)}catch(o){console.error(`[save-hook] Unhandled error: ${o.message}`),console.log(N("PostToolUse",!1,{reason:o.message})),process.exit(1)}});async function qe(o){if(!o)throw new Error("newHook requires input");let{session_id:e,cwd:s,prompt:t,transcript_path:r}=o;E("[new-hook] Input received",{session_id:e,cwd:s,cwd_type:typeof s,cwd_length:s?.length,has_cwd:!!s,prompt_length:t?.length});let n=ze.basename(s);E("[new-hook] Project extracted",{project:n,project_type:typeof n,project_length:n?.length,is_empty:n==="",cwd_was:s}),await w();let i=new C,a=i.createSDKSession(e,n,t),d=i.incrementPromptCounter(a);if(i.saveUserPrompt(e,d,t),console.error(`[new-hook] Session ${a}, prompt #${d}`),i.close(),k.getConfig().enabled&&r)try{l.info("HOOK","\u{1F504} Batch transforming transcript at UserPromptSubmit",{transcriptPath:r});let m=await ne(r,`user-prompt-${d}`);l.success("HOOK","\u2705 Batch transformation complete",{originalTokens:m.originalTokens,compressedTokens:m.compressedTokens,savings:`${Math.round((1-m.compressedTokens/m.originalTokens)*100)}%`})}catch(m){l.warn("HOOK","Batch transformation failed - continuing anyway",{transcriptPath:r},m)}let S=U(),T=t.startsWith("/")?t.substring(1):t;try{let m=await fetch(`http://127.0.0.1:${S}/sessions/${a}/init`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:n,userPrompt:T,promptNumber:d}),signal:AbortSignal.timeout(5e3)});if(!m.ok){let p=await m.text();throw new Error(`Failed to initialize session: ${m.status} ${p}`)}}catch(m){throw m.cause?.code==="ECONNREFUSED"||m.name==="TimeoutError"||m.message.includes("fetch failed")?new Error("There's a problem with the worker. If you just updated, type `pm2 restart claude-mem-worker` in your terminal to continue"):m}console.log(N("UserPromptSubmit",!0))}var Y="";ie.on("data",o=>Y+=o);ie.on("end",async()=>{let o=Y?JSON.parse(Y):void 0;await qe(o)});
