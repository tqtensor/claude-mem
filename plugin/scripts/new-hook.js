...
`)+`
`;Ve(P,T,"utf8")}var X=100;function le(r){let e=(r.match(/<private>/g)||[]).length,s=(r.match(/<claude-mem-context>/g)||[]).length;return e+s}function ee(r){if(typeof r!="string")return D("[tag-stripping] received non-string for JSON context:",{type:typeof r}),"{}";let e=le(r);return e>X&&D("[tag-stripping] tag count exceeds limit, truncating:",{tagCount:e,maxAllowed:X,contentLength:r.length}),r.replace(/<claude-mem-context>[\s\S]*?<\/claude-mem-context>/g,"").replace(/<private>[\s\S]*?<\/private>/g,"").trim()}function ue(r){if(typeof r!="string")return D("[tag-stripping] received non-string for prompt context:",{type:typeof r}),"";let e=le(r);return e>X&&D("[tag-stripping] tag count exceeds limit, truncating:",{tagCount:e,maxAllowed:X,contentLength:r.length}),r.replace(/<claude-mem-context>[\s\S]*?<\/claude-mem-context>/g,"").replace(/<private>[\s\S]*?<\/private>/g,"").trim()}import{readFileSync as se,writeFileSync as Ee,renameSync as ss,copyFileSync as ts}from"fs";import{randomUUID as me}from"crypto";async function ge(r,e){try{U(x);let p=j(r);ts(r,p),d.debug("HOOK","Created transcript backup before clearing input",{original:r,backup:p})}catch(p){throw d.error("HOOK","Failed to create transcript backup",{transcriptPath:r},p),new Error("Backup creation failed - aborting transformation for safety")}let t=se(r,"utf-8").trim().split(`
...
`)+`
`,"utf-8");let c=se(i,"utf-8").trim().split(`
`);for(let p of c)p.trim()&&JSON.parse(p);return ss(i,r),o}async function Te(r,e,s,t){if(t.length===0){d.debug("HOOK","No observations to inject");return}let o=se(r,"utf-8"),n=o.trim().split(`
`),i=JSON.parse(n[n.length-1]),a={isSidechain:!1,userType:i.userType||"user",cwd:s,sessionId:e,version:i.version||"1.0",timestamp:new Date().toISOString()},c=`toolu_mem_${Date.now()}`,p={...a,type:"assistant",uuid:me(),message:{id:`msg_${Date.now()}`,type:"message",role:"assistant",model:"claude-3-5-sonnet-20241022",content:[{type:"tool_use",id:c,name:"claude-mem-fetch-observations",input:{observation_ids:t.map(u=>u.id),note:"Fetching compressed observations for context"}}],stop_reason:"tool_use"}},T=t.map(u=>rs(u)).join(`

---

`),_={...a,type:"user",uuid:me(),message:{role:"user",content:[{type:"tool_result",tool_use_id:c,content:T}]}},f=[JSON.stringify(p),JSON.stringify(_)];Ee(r,o+`
`+f.join(`
`)+`
