#!/usr/bin/env node
import Se from"path";import{stdin as Y}from"process";import Q from"better-sqlite3";import{join as T,dirname as K,basename as he}from"path";import{homedir as U}from"os";import{existsSync as Ae,mkdirSync as J}from"fs";import{fileURLToPath as q}from"url";function V(){return typeof __dirname<"u"?__dirname:K(q(import.meta.url))}var z=V(),g=process.env.CLAUDE_MEM_DATA_DIR||T(U(),".claude-mem"),v=process.env.CLAUDE_CONFIG_DIR||T(U(),".claude"),ve=T(g,"archives"),Ce=T(g,"logs"),ke=T(g,"trash"),C=T(g,"backups"),De=T(g,"settings.json"),w=T(g,"claude-mem.db"),Me=T(g,"vector-db"),xe=T(v,"settings.json"),Ue=T(v,"commands"),we=T(v,"CLAUDE.md");function h(n){J(n,{recursive:!0})}function F(){return T(z,"..","..")}var k=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(k||{}),D=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=k[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let r=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${r})`}if(e==="Read"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Edit"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}if(e==="Write"&&t.file_path){let r=t.file_path.split("/").pop()||t.file_path;return`${e}(${r})`}return e}catch{return e}}log(e,s,t,r,o){if(e<this.level)return;let i=new Date().toISOString().replace("T"," ").substring(0,23),a=k[e].padEnd(5),d=s.padEnd(6),p="";r?.correlationId?p=`[${r.correlationId}] `:r?.sessionId&&(p=`[session-${r.sessionId}] `);let u="";o!=null&&(this.level===0&&typeof o=="object"?u=`
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let E="";if(r){let{sessionId:b,sdkSessionId:f,correlationId:l,...c}=r;Object.keys(c).length>0&&(E=` {${Object.entries(c).map(([O,L])=>`${O}=${L}`).join(", ")}}`)}let _=`[${i}] [${a}] [${d}] ${p}${t}${E}${u}`;e===3?console.error(_):console.log(_)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},m=new D;var S=class{db;constructor(){h(g),this.db=new Q(w),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.addEndlessModeStatsColumns(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.addToolUseIdColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r}=s,o=t==="date_asc"?"ASC":"DESC",i=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${i}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,p;if(e!==null){let b=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let l=this.db.prepare(b).all(e,...a,t+1),c=this.db.prepare(f).all(e,...a,r+1);if(l.length===0&&c.length===0)return{observations:[],sessions:[],prompts:[]};d=l.length>0?l[l.length-1].created_at_epoch:s,p=c.length>0?c[c.length-1].created_at_epoch:s}catch(l){return console.error("[SessionStore] Error getting boundary observations:",l.message),{observations:[],sessions:[],prompts:[]}}}else{let b=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let l=this.db.prepare(b).all(s,...a,t),c=this.db.prepare(f).all(s,...a,r+1);if(l.length===0&&c.length===0)return{observations:[],sessions:[],prompts:[]};d=l.length>0?l[l.length-1].created_at_epoch:s,p=c.length>0?c[c.length-1].created_at_epoch:s}catch(l){return console.error("[SessionStore] Error getting boundary timestamps:",l.message),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
    ...
    `;try{let b=this.db.prepare(u).all(d,p,...a),f=this.db.prepare(E).all(d,p,...a),l=this.db.prepare(_).all(d,p,...a);return{observations:b,sessions:f.map(c=>({id:c.id,sdk_session_id:c.sdk_session_id,project:c.project,request:c.request,completed:c.completed,next_steps:c.next_steps,created_at:c.created_at,created_at_epoch:c.created_at_epoch})),prompts:l.map(c=>({id:c.id,claude_session_id:c.claude_session_id,project:c.project,prompt:c.prompt_text,created_at:c.created_at,created_at_epoch:c.created_at_epoch}))}}catch(b){return console.error("[SessionStore] Error querying timeline records:",b.message),{observations:[],sessions:[],prompts:[]}}}incrementEndlessModeStats(e,s,t){let r=s-t;this.db.prepare(`
      UPDATE sdk_sessions
      SET
...
Or restart: pm2 restart claude-mem-worker`)}}import{existsSync as ae,readFileSync as de}from"fs";import{homedir as ce}from"os";import pe from"path";function I(n,e,s){if(n!==void 0){if(typeof n=="boolean")return n;if(typeof n=="string")return n.toLowerCase()==="true"}return e!==void 0?e.toLowerCase()==="true":s}function M(n,e,s){if(n!==void 0){if(typeof n=="number")return n;if(typeof n=="string"){let t=parseInt(n,10);if(!isNaN(t))return t}}if(e!==void 0){let t=parseInt(e,10);if(!isNaN(t))return t}return s}function ue(){let n=pe.join(ce(),".claude-mem","settings.json"),e={};if(ae(n))try{e=JSON.parse(de(n,"utf-8"))}catch(u){m.warn("CONFIG","Failed to parse settings.json, using environment/defaults",{},u)}let s=I(e.env?.CLAUDE_MEM_ENDLESS_MODE,process.env.CLAUDE_MEM_ENDLESS_MODE,!1),t=I(e.env?.CLAUDE_MEM_TRANSFORM_FALLBACK,process.env.CLAUDE_MEM_TRANSFORM_FALLBACK,!0),r=M(e.env?.CLAUDE_MEM_TRANSFORM_TIMEOUT,process.env.CLAUDE_MEM_TRANSFORM_TIMEOUT,500),o=M(e.env?.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,process.env.CLAUDE_MEM_TRANSFORM_KEEP_RECENT,0),i=I(e.env?.CLAUDE_MEM_OBSERVE_EVERYTHING,process.env.CLAUDE_MEM_OBSERVE_EVERYTHING,s),a=M(e.env?.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,process.env.CLAUDE_MEM_ENDLESS_MODE__MAX_TOOL_HISTORY__MB,50),d=I(e.env?.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,process.env.CLAUDE_MEM_ENABLE_SYNCHRONOUS_MODE,s),p={enabled:s,fallbackToOriginal:t,maxLookupTime:r,keepRecentToolUses:o,observeEverything:i,maxToolHistoryMB:a,enableSynchronousMode:d};return s?m.info("CONFIG","Endless Mode enabled",{fallback:t,maxLookupTime:`${r}ms`,keepRecent:o,observeEverything:i,maxToolHistoryMB:`${a}MB`}):m.debug("CONFIG","Endless Mode disabled"),p}var y=class{static getConfig=ue;static clearCache(){}};import{readFileSync as G,writeFileSync as Ee,renameSync as Te}from"fs";import{existsSync as ts,readFileSync as rs,writeFileSync as ns,appendFileSync as _e,statSync as os}from"fs";import{join as le}from"path";var me=le(C,"tool-outputs.jsonl");function j(n,e,s=Date.now()){h(C);let t=typeof e=="string"?e:JSON.stringify(e),r=Buffer.byteLength(t,"utf8"),i=JSON.stringify({tool_use_id:n,content:e,timestamp:s,size_bytes:r})+`
`;_e(me,i,"utf8")}function A(n,e){if(!n)return[];if(Array.isArray(n))return n;try{let s=JSON.parse(n);return Array.isArray(s)?s:[]}catch(s){return m.debug("HOOK",`Failed to parse ${e}`,{field:n,error:s}),[]}}function be(n){try{let s=G(n,"utf-8").trim().split(`
`),t=[];for(let r of s)if(r.trim())try{let o=JSON.parse(r);if(o.type==="user"){let a=o.message.content;if(Array.isArray(a)){for(let d of a)if(d.type==="tool_result"){let p=d;!(typeof p.content=="string"?p.content:JSON.stringify(p.content)).trim().startsWith("# ")&&p.tool_use_id&&t.push(p.tool_use_id)}}}}catch{continue}return t}catch(e){return m.warn("HOOK","Failed to extract pending tool_use_ids",{transcriptPath:n},e),[]}}function fe(n){let e=A(n.facts,"facts"),s=A(n.concepts,"concepts"),t=A(n.files_read,"files_read"),r=A(n.files_modified,"files_modified");return`# ${n.title}
...
`:""}
---
*[Compressed by Endless Mode]*`}async function ge(n,e,s){let r=G(n,"utf-8").trim().split(`
`),o=!1,i=0,a=0,d=r.map((u,E)=>{if(!u.trim())return u;try{let _=JSON.parse(u);if(_.type==="user"){let f=_.message.content;if(Array.isArray(f))for(let l=0;l<f.length;l++){let c=f[l];if(c.type==="tool_result"){let R=c;if(R.tool_use_id===e){o=!0;try{j(e,R.content,Date.now()),m.debug("HOOK","Backed up original tool output",{toolUseId:e})}catch(L){m.warn("HOOK","Failed to backup original tool output",{toolUseId:e},L)}i=JSON.stringify(R.content).length;let O=fe(s);a=O.length,R.content=O,m.success("HOOK","Transformed tool result",{toolUseId:e,originalSize:i,compressedSize:a,savings:`${Math.round((1-a/i)*100)}%`})}}}}return JSON.stringify(_)}catch(_){throw m.warn("HOOK","Malformed JSONL line in transcript",{lineIndex:E,error:_}),new Error(`Malformed JSONL line at index ${E}: ${_.message}`)}});if(!o)return m.warn("HOOK","Tool result not found in transcript",{toolUseId:e}),{originalTokens:0,compressedTokens:0};let p=`${n}.tmp`;return Ee(p,d.join(`
`)+`
`,"utf-8"),Te(p,n),{originalTokens:i,compressedTokens:a}}async function W(n,e,s){let t=0;try{let r=be(n);if(r.length===0)return 0;m.debug(s,"Found pending tool_use_ids",{count:r.length,ids:r});let o=new S,i=o.getObservationsByToolUseIds(r);if(o.close(),i.size===0)return 0;m.info(s,"Ready observations for transformation",{pending:r.length,ready:i.size});for(let[a,d]of i)try{let p={id:d.id,type:d.type,title:d.title,subtitle:d.subtitle,narrative:d.narrative,facts:JSON.parse(d.facts),concepts:JSON.parse(d.concepts),files_read:JSON.parse(d.files_read),files_modified:JSON.parse(d.files_modified),created_at_epoch:d.created_at_epoch},u=await ge(n,a,p);if(u.originalTokens>0)try{let E=new S;E.incrementEndlessModeStats(e,u.originalTokens,u.compressedTokens),E.close()}catch(E){m.debug(s,"Stats update skipped",{error:E})}m.success(s,"Deferred transformation complete",{toolUseId:a,observationId:d.id,savings:`${Math.round((1-u.compressedTokens/u.originalTokens)*100)}%`}),t++}catch(p){m.warn(s,"Deferred transformation failed",{toolUseId:a},p)}}catch(r){m.warn(s,"Deferred transformation check failed",{},r)}return t}async function Re(n){if(!n)throw new Error("newHook requires input");let{session_id:e,cwd:s,prompt:t}=n,r=Se.basename(s);await $();let o=new S,i=o.createSDKSession(e,r,t),a=o.incrementPromptCounter(i);o.saveUserPrompt(e,a,t),console.error(`[new-hook] Session ${i}, prompt #${a}`),o.close();let d=y.getConfig().enabled,p=n.transcript_path;d&&p&&await W(p,e,"NEW_HOOK");let u=N(),E=t.startsWith("/")?t.substring(1):t;try{let _=await fetch(`http://127.0.0.1:${u}/sessions/${i}/init`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:r,userPrompt:E,promptNumber:a}),signal:AbortSignal.timeout(5e3)});if(!_.ok){let b=await _.text();throw new Error(`Failed to initialize session: ${_.status} ${b}`)}}catch(_){throw _.cause?.code==="ECONNREFUSED"||_.name==="TimeoutError"||_.message.includes("fetch failed")?new Error("There's a problem with the worker. If you just updated, type `pm2 restart claude-mem-worker` in your terminal to continue"):_}console.log(B("UserPromptSubmit",!0))}var x="";Y.on("data",n=>x+=n);Y.on("end",async()=>{let n=x?JSON.parse(x):void 0;await Re(n)});
