...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let _="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(_=` {${Object.entries(l).map(([S,N])=>`${S}=${N}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${m}${s}${_}${u}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=m?`${m[1].split("/").pop()}:${m[2]}`:"unknown",_={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},c=new W;var Ct={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(Ct.url))}var Rt=Ot(),A=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),Yt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),Vt=(0,b.join)(A,"archives"),qt=(0,b.join)(A,"logs"),Kt=(0,b.join)(A,"trash"),Jt=(0,b.join)(A,"backups"),zt=(0,b.join)(A,"modes"),Qt=(0,b.join)(A,"settings.json"),ue=(0,b.join)(A,"claude-mem.db"),Zt=(0,b.join)(A,"vector-db"),es=(0,b.join)(A,"observer-sessions"),ts=(0,b.join)(y,"settings.json"),ss=(0,b.join)(y,"commands"),rs=(0,b.join)(y,"CLAUDE.md");function _e(r){(0,ce.mkdirSync)(r,{recursive:!0,mode:448})}function le(){return(0,b.join)(Rt,"..")}var $=class{db;constructor(e=ue){e!==":memory:"&&_e(A),this.db=new Ee.Database(e),e!==":memory:"&&(0,Te.chmodSync)(e,384),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.ensureSourceToolColumns()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),c.debug("DB","pending_messages table created successfully")}renameSessionIdColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(17))return;c.debug("DB","Checking session ID columns for semantic clarity rename");let t=0,s=(n,o,i)=>{let a=this.db.query(`PRAGMA table_info(${n})`).all(),d=a.some(u=>u.name===o);return a.some(u=>u.name===i)?!1:d?(this.db.run(`ALTER TABLE ${n} RENAME COLUMN ${o} TO ${i}`),c.debug("DB",`Renamed ${n}.${o} to ${i}`),!0):(c.warn("DB",`Column ${o} not found in ${n}, skipping rename`),!1)};s("sdk_sessions","claude_session_id","content_session_id")&&t++,s("sdk_sessions","sdk_session_id","memory_session_id")&&t++,s("pending_messages","claude_session_id","content_session_id")&&t++,s("observations","sdk_session_id","memory_session_id")&&t++,s("session_summaries","sdk_session_id","memory_session_id")&&t++,s("user_prompts","claude_session_id","content_session_id")&&t++,this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(17,new Date().toISOString()),t>0?c.debug("DB",`Successfully renamed ${t} session ID columns`):c.debug("DB","No session ID column renames needed (already up to date)")}repairSessionIdColumnRename(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(19)||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(19,new Date().toISOString())}addFailedAtEpochColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(20))return;this.db.query("PRAGMA table_info(pending_messages)").all().some(n=>n.name==="failed_at_epoch")||(this.db.run("ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER"),c.debug("DB","Added failed_at_epoch column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(20,new Date().toISOString())}addOnUpdateCascadeToForeignKeys(){if(!this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)){c.debug("DB","Adding ON UPDATE CASCADE to FK constraints on observations and session_summaries"),this.db.run("PRAGMA foreign_keys = OFF"),this.db.run("BEGIN TRANSACTION");try{this.db.run("DROP TRIGGER IF EXISTS observations_ai"),this.db.run("DROP TRIGGER IF EXISTS observations_ad"),this.db.run("DROP TRIGGER IF EXISTS observations_au"),this.db.run(`
      ...
      `),this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_summaries_fts'").all().length>0&&this.db.run(`
          ...
          CREATE TRIGGER IF NOT EXISTS session_summaries_au AFTER UPDATE ON session_summaries BEGIN
            ...
            VALUES (new.id, new.request, new.investigated, new.learned, new.completed, new.next_steps, new.notes);
          END;
        `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),this.db.run("COMMIT"),this.db.run("PRAGMA foreign_keys = ON"),c.debug("DB","Successfully added ON UPDATE CASCADE to FK constraints")}catch(t){throw this.db.run("ROLLBACK"),this.db.run("PRAGMA foreign_keys = ON"),t}}}ensureSourceToolColumns(){let e=this.db.query("PRAGMA table_info(observations)").all(),t=e.some(n=>n.name==="source_tool"),s=e.some(n=>n.name==="source_input_summary");t&&s||(t||(this.db.run("ALTER TABLE observations ADD COLUMN source_tool TEXT"),c.debug("DB","Added source_tool column to observations table")),s||(this.db.run("ALTER TABLE observations ADD COLUMN source_input_summary TEXT"),c.debug("DB","Added source_input_summary column to observations table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString()))}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,m=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let l=Array.isArray(a)?a:[a],f=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...l),E.push(`(${f.join(" OR ")})`)}if(d){let l=Array.isArray(d)?d:[d],f=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(S=>{T.push(`%${S}%`,`%${S}%`)}),E.push(`(${f.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,s.source_tool||null,s.source_input_summary||null,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),u=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),m=new Date(d).toISOString();return this.db.transaction(()=>{let _=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, source_tool, source_input_summary,
         created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,g.source_tool||null,g.source_input_summary||null,m,d);_.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,d);E=Number(h.lastInsertRowid)}return{observationIds:_,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,m){let u=m??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, source_tool, source_input_summary,
         created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=g.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,f.source_tool||null,f.source_input_summary||null,_,u);E.push(Number(S.lastInsertRowid))}let h;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,_,u);h=Number(S.lastInsertRowid)}return this.db.prepare(`
    ...
    `).all(...m)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,m;if(e!==null){let l=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let S=this.db.prepare(l).all(e,...a,s+1),N=this.db.prepare(f).all(e,...a,n+1);if(S.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,m=N.length>0?N[N.length-1].created_at_epoch:t}catch(S){return c.error("DB","Error getting boundary observations",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let S=this.db.prepare(l).all(t,...a,s),N=this.db.prepare(f).all(t,...a,n+1);if(S.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,m=N.length>0?N[N.length-1].created_at_epoch:t}catch(S){return c.error("DB","Error getting boundary timestamps",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...xe(r));let a=null,d="",m=!1;for(let u of e)if(u.type==="summary"){m&&(i.push(""),m=!1,a=null,d="");let _=u.data,T=rt(_.displayTime);o?i.push(...ze(_,T)):i.push(...$e(_,T))}else{let _=u.data,T=it(_.files_modified,n,_.files_read),E=nt(_.created_at),g=E!==d,h=g?E:"";d=E;let l=t.has(_.id);if(T!==a&&(m&&i.push(""),o?i.push(...qe(T)):i.push(...Ue(T)),a=T,m=!0),l){let f=vt(_,s);o?i.push(...Je(_,E,g,f,s)):(m&&!o&&(i.push(""),m=!1),i.push(...we(_,h,f,s)),a=null)}else o?i.push(Ke(_,E,g,s)):i.push(ke(_,h,s))}return m&&i.push(""),i}function at(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,d]of i)o.push(...Lt(a,d,e,t,s,n));return o}function dt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,p.blue)),t.push(...U("Learned",r.learned,p.yellow)),t.push(...U("Completed",r.completed,p.green)),t.push(...U("Next Steps",r.next_steps,p.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):Fe(r)}function mt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ze(r.totalDiscoveryTokens,r.totalReadTokens):Pe(r.totalDiscoveryTokens,r.totalReadTokens)}var Dt=ut.default.join((0,_t.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function xt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,lt.unlinkSync)(Dt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?et(r):Xe(r)}function kt(r,e,t,s,n,o,i){let a=[],d=K(e);a.push(...tt(r,d,s,i));let m=t.slice(0,s.sessionCount),u=Ne(m,t),_=Z(e,u),T=Ae(e,s.fullObservationCount);a.push(...at(_,T,s,n,i));let E=t[0],g=e[0];dt(s,E,g)&&a.push(...pt(E,i));let h=Q(e,s,o,n);return a.push(...ct(h,i)),a.push(...mt(d,s,i)),a.join(`
