...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:h,correlationId:S,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([f,O])=>`${f}=${O}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${p}${s}${u}${l}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},m=new W;var Lt={};function It(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(Lt.url))}var yt=It(),N=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,ce.homedir)(),".claude"),Zt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),es=(0,b.join)(N,"archives"),ts=(0,b.join)(N,"logs"),ss=(0,b.join)(N,"trash"),rs=(0,b.join)(N,"backups"),ns=(0,b.join)(N,"modes"),os=(0,b.join)(N,"settings.json"),le=(0,b.join)(N,"claude-mem.db"),is=(0,b.join)(N,"vector-db"),as=(0,b.join)(N,"observer-sessions"),ds=(0,b.join)(y,"settings.json"),cs=(0,b.join)(y,"commands"),ps=(0,b.join)(y,"CLAUDE.md");function ue(r){(0,pe.mkdirSync)(r,{recursive:!0})}function _e(){return(0,b.join)(yt,"..")}var Ee=require("crypto");var vt=3e4;function Te(r,e,t,s){return(0,Ee.createHash)("sha256").update((r||"")+(s||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function ge(r,e,t){let s=t-vt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=le){e!==":memory:"&&ue(N),this.db=new fe.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addObservationBranchColumns()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d,commit_sha:p}=t,l=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",g=e.map(()=>"?").join(","),E=[...e],h=[];if(o&&(h.push("project = ?"),E.push(o)),i)if(Array.isArray(i)){let T=i.map(()=>"?").join(",");h.push(`type IN (${T})`),E.push(...i)}else h.push("type = ?"),E.push(i);if(a){let T=Array.isArray(a)?a:[a],f=T.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");E.push(...T),h.push(`(${f.join(" OR ")})`)}if(d){let T=Array.isArray(d)?d:[d],f=T.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");T.forEach(O=>{E.push(`%${O}%`,`%${O}%`)}),h.push(`(${f.join(" OR ")})`)}if(p)if(Array.isArray(p)){let T=p.map(()=>"?").join(",");h.push(`(commit_sha IS NULL OR commit_sha IN (${T}))`),E.push(...p)}else h.push("(commit_sha IS NULL OR commit_sha = ?)"),E.push(p);let S=h.length>0?`WHERE id IN (${g}) AND ${h.join(" AND ")}`:`WHERE id IN (${g})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${S}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=Te(e,s.title,s.narrative),l=ge(this.db,p,a);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let g=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d,a);return{id:Number(g.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d,p){let l=a??Date.now(),u=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch,
         branch, commit_sha)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let _ of s){let T=h.run(e,t,_.type,_.title,_.subtitle,JSON.stringify(_.facts),_.narrative,JSON.stringify(_.concepts),JSON.stringify(_.files_read),JSON.stringify(_.files_modified),o||null,i,u,l,d??null,p??null);E.push(Number(T.lastInsertRowid))}let S=null;if(n){let T=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,u,l);S=Number(T.lastInsertRowid)}return{observationIds:E,summaryId:S,createdAtEpoch:l}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let l=p??Date.now(),u=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let f=h.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),a||null,d,u,l);E.push(Number(f.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,l);S=Number(f.lastInsertRowid)}return this.db.prepare(`
  ...
  `).all(e,t.sessionCount+q)}function ye(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(","),d=Ie(s),p=e.map(()=>"?").join(",");return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(...e,...n,...i,...d.params,t.totalObservationCount)}function Le(r,e,t){let s=e.map(()=>"?").join(",");return r.db.prepare(`
