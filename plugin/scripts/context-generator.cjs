...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),S=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",b={...t,location:S};return this.warn(e,`[HAPPY-PATH] ${s}`,b,r),i}},R=new me});var G,ke,Ue,$,ee=K(()=>{"use strict";G=require("fs"),ke=require("path"),Ue=require("os");ue();B();$=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,ke.join)((0,Ue.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:De,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:Me,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,G.existsSync)(e))return this.getAllDefaults();let s=(0,G.readFileSync)(e,"utf-8"),t=JSON.parse(s),r=t;if(t.env&&typeof t.env=="object"){r=t.env;try{(0,G.writeFileSync)(e,JSON.stringify(r,null,2),"utf-8"),R.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(d){R.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},d)}}let i={...this.DEFAULTS};for(let d of Object.keys(this.DEFAULTS))r[d]!==void 0&&(i[d]=r[d]);return i}}});function cs(){return typeof __dirname<"u"?__dirname:(0,O.dirname)((0,we.fileURLToPath)(_s.url))}function je(a){(0,xe.mkdirSync)(a,{recursive:!0})}function Pe(){return(0,O.join)(ps,"..","..")}var O,$e,xe,we,_s,ps,y,Ee,Ms,ks,Us,$s,xs,ws,Fe,Fs,js,Ps,Xs,Te=K(()=>{"use strict";O=require("path"),$e=require("os"),xe=require("fs"),we=require("url");ee();_s={};ps=cs(),y=$.get("CLAUDE_MEM_DATA_DIR"),Ee=process.env.CLAUDE_CONFIG_DIR||(0,O.join)((0,$e.homedir)(),".claude"),Ms=(0,O.join)(y,"archives"),ks=(0,O.join)(y,"logs"),Us=(0,O.join)(y,"trash"),$s=(0,O.join)(y,"backups"),xs=(0,O.join)(y,"modes"),ws=(0,O.join)(y,"settings.json"),Fe=(0,O.join)(y,"claude-mem.db"),Fs=(0,O.join)(y,"vector-db"),js=(0,O.join)(Ee,"settings.json"),Ps=(0,O.join)(Ee,"commands"),Xs=(0,O.join)(Ee,"CLAUDE.md")});var Ke={};Le(Ke,{ModeManager:()=>be});var te,Se,be,qe=K(()=>{"use strict";te=require("fs"),Se=require("path");B();Te();be=class a{static instance=null;modeCache=new Map;modesDir;constructor(){this.modesDir=(0,Se.join)(Pe(),"modes")}static getInstance(){return a.instance||(a.instance=new a),a.instance}loadMode(e){let s=this.modeCache.get(e);if(s)return s;let t=(0,Se.join)(this.modesDir,`${e}.json`);if(!(0,te.existsSync)(t)){if(R.warn("SYSTEM",`Mode file not found: ${t}, falling back to 'code'`),e==="code")throw new Error("Critical: code.json mode file missing");return this.loadMode("code")}try{let r=(0,te.readFileSync)(t,"utf-8"),i=JSON.parse(r);return this.modeCache.set(e,i),R.debug("SYSTEM",`Loaded mode: ${i.name} (${e})`,void 0,{types:i.observation_types.map(d=>d.id),concepts:i.observation_concepts.map(d=>d.id)}),i}catch(r){if(R.error("SYSTEM",`Failed to load mode: ${t}`,void 0,r),e==="code")throw r;return this.loadMode("code")}}getObservationTypes(e){return this.loadMode(e).observation_types}getObservationConcepts(e){return this.loadMode(e).observation_concepts}getTypeIcon(e,s){return this.getObservationTypes(e).find(r=>r.id===s)?.emoji||"\u{1F4DD}"}getWorkEmoji(e,s){return this.getObservationTypes(e).find(r=>r.id===s)?.work_emoji||"\u{1F4DD}"}validateType(e,s){return this.getObservationTypes(e).some(t=>t.id===s)}getTypeLabel(e,s){return this.getObservationTypes(e).find(r=>r.id===s)?.label||s}}});var Ss={};Le(Ss,{generateContext:()=>hs});module.exports=ds(Ss);var ne=ce(require("path"),1),oe=require("os"),Y=require("fs");var Xe=require("bun:sqlite");Te();B();var se=class{db;constructor(){je(y),this.db=new Xe.Database(Fe),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.ensureMetadataJsonColumn()}initializeSchema(){try{this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:i,type:d,concepts:c,files:u}=s,l=t==="date_asc"?"ASC":"DESC",S=r?`LIMIT ${r}`:"",b=e.map(()=>"?").join(","),f=[...e],E=[];if(i&&(E.push("project = ?"),f.push(i)),d)if(Array.isArray(d)){let p=d.map(()=>"?").join(",");E.push(`type IN (${p})`),f.push(...d)}else E.push("type = ?"),f.push(d);if(c){let p=Array.isArray(c)?c:[c],L=p.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");f.push(...p),E.push(`(${L.join(" OR ")})`)}if(u){let p=Array.isArray(u)?u:[u],L=p.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");p.forEach(j=>{f.push(`%${j}%`,`%${j}%`)}),E.push(`(${L.join(" OR ")})`)}let I=E.length>0?`WHERE id IN (${b}) AND ${E.join(" AND ")}`:`WHERE id IN (${b})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${I}
    ...
    `).get(e)?.prompt_counter||1}getPromptCounter(e){return this.db.prepare(`
      SELECT prompt_counter FROM sdk_sessions WHERE id = ?
    `).get(e)?.prompt_counter||0}createSDKSession(e,s,t,r){let i=new Date,d=i.getTime(),c=r?JSON.stringify({mode:r}):null,l=this.db.prepare(`
      INSERT OR IGNORE INTO sdk_sessions
      (claude_session_id, sdk_session_id, project, user_prompt, started_at, started_at_epoch, status, metadata_json)
      VALUES (?, ?, ?, ?, ?, ?, 'active', ?)
    `).run(e,e,s,t,i.toISOString(),d,c);if(l.lastInsertRowid===0||l.changes===0){if(s&&s.trim()!==""&&this.db.prepare(`
          UPDATE sdk_sessions
    ...
    `).get(e)||(this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, 'active')
      `).run(e,e,s,d.toISOString(),c),console.log(`[SessionStore] Auto-created session record for session_id: ${e}`));let b=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),r||null,i,d.toISOString(),c);return{id:Number(b.lastInsertRowid),createdAtEpoch:c}}storeSummary(e,s,t,r,i=0){let d=new Date,c=d.getTime();this.db.prepare(`
      SELECT id FROM sdk_sessions WHERE sdk_session_id = ?
    `).get(e)||(this.db.prepare(`
        INSERT INTO sdk_sessions
        (claude_session_id, sdk_session_id, project, started_at, started_at_epoch, status)
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||null,i,d.toISOString(),c);return{id:Number(b.lastInsertRowid),createdAtEpoch:c}}markSessionCompleted(e){let s=new Date,t=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).run(s.toISOString(),t,e)}markSessionFailed(e){let s=new Date,t=s.getTime();this.db.prepare(`
      ...
      WHERE id = ?
    `).run(s.toISOString(),t,e)}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:i}=s,d=t==="date_asc"?"ASC":"DESC",c=r?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),l=[...e],S=i?`WHERE id IN (${u}) AND project = ?`:`WHERE id IN (${u})`;return i&&l.push(i),this.db.prepare(`
      SELECT * FROM session_summaries
      ${S}
      ORDER BY created_at_epoch ${d}
      ${c}
    `).all(...l)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:i}=s,d=t==="date_asc"?"ASC":"DESC",c=r?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),l=[...e],S=i?"AND s.project = ?":"";return i&&l.push(i),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
      ...
      ${c}
    `).all(...l)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,i){let d=i?"AND project = ?":"",c=i?[i]:[],u,l;if(e!==null){let E=`
    ...
    `).run(e.claude_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}};ue();B();ee();var ge=ce(require("path"),1);function he(a){if(!a)return[];try{let e=JSON.parse(a);return Array.isArray(e)?e:[]}catch{return[]}}function We(a){return new Date(a).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function He(a){return new Date(a).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Be(a){return new Date(a).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function us(a,e){return ge.default.isAbsolute(a)?ge.default.relative(e,a):a}function Ge(a,e){let s=he(a);return s.length>0?us(s[0],e):"General"}var Ye=ce(require("path"),1);B();function Ve(a){if(!a||a.trim()==="")return R.warn("PROJECT_NAME","Empty cwd provided, using fallback",{cwd:a}),"unknown-project";let e=Ye.default.basename(a);if(e==="")if(process.platform==="win32"&&a.match(/^[A-Z]:\\/i)){let r=`drive-${a[0].toUpperCase()}`;return R.info("PROJECT_NAME","Drive root detected",{cwd:a,projectName:r}),r}else return R.warn("PROJECT_NAME","Root directory detected, using fallback",{cwd:a}),"unknown-project";return e}var ls=ne.default.join((0,oe.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function ms(){let a=ne.default.join((0,oe.homedir)(),".claude-mem","settings.json"),e=$.loadFromFile(a);try{return{totalObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10),fullObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_FULL_COUNT,10),sessionCount:parseInt(e.CLAUDE_MEM_CONTEXT_SESSION_COUNT,10),showReadTokens:e.CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS==="true",showWorkTokens:e.CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS==="true",showSavingsAmount:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT==="true",showSavingsPercent:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT==="true",observationTypes:new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES.split(",").map(s=>s.trim()).filter(Boolean)),observationConcepts:new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS.split(",").map(s=>s.trim()).filter(Boolean)),fullObservationField:e.CLAUDE_MEM_CONTEXT_FULL_FIELD,showLastSummary:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY==="true",showLastMessage:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE==="true"}}catch(s){return R.warn("WORKER","Failed to load context settings, using defaults",{},s),{totalObservationCount:50,fullObservationCount:5,sessionCount:10,showReadTokens:!0,showWorkTokens:!0,showSavingsAmount:!0,showSavingsPercent:!0,observationTypes:new Set(pe),observationConcepts:new Set(_e),fullObservationField:"narrative",showLastSummary:!0,showLastMessage:!1}}}var Je=4,Es=1,o={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function re(a,e,s,t){return e?t?[`${s}${a}:${o.reset} ${e}`,""]:[`**${a}**: ${e}`,""]:[]}function Ts(a){return a.replace(/\//g,"-")}function gs(a){try{if(!(0,Y.existsSync)(a))return{userMessage:"",assistantMessage:""};let e=(0,Y.readFileSync)(a,"utf-8").trim();if(!e)return{userMessage:"",assistantMessage:""};let s=e.split(`
`).filter(r=>r.trim()),t="";for(let r=s.length-1;r>=0;r--)try{let i=s[r];if(!i.includes('"type":"assistant"'))continue;let d=JSON.parse(i);if(d.type==="assistant"&&d.message?.content&&Array.isArray(d.message.content)){let c="";for(let u of d.message.content)u.type==="text"&&(c+=u.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){t=c;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return R.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:a},e),{userMessage:"",assistantMessage:""}}}async function hs(a,e=!1){let s=ms(),t=a?.cwd??process.cwd(),r=Ve(t),i=a?.mode||"code",d=null;try{d=new se}catch(A){if(A.code==="ERR_DLOPEN_FAILED"){try{(0,Y.unlinkSync)(ls)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw A}let{ModeManager:c}=await Promise.resolve().then(()=>(qe(),Ke)),l=c.getInstance().loadMode(i),S=l.observation_types.map(A=>A.id),b=l.observation_concepts.map(A=>A.id),f=S,E=f.map(()=>"?").join(","),I=b,T=I.map(()=>"?").join(","),p=d.db.prepare(`
    SELECT
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(r,...f,...I,s.totalObservationCount),L=d.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(r,s.sessionCount+Es),j="",P="";if(s.showLastMessage&&p.length>0)try{let A=a?.session_id,D=p.find(N=>N.sdk_session_id!==A);if(D){let N=D.sdk_session_id,M=Ts(t),X=ne.default.join((0,oe.homedir)(),".claude","projects",M,`${N}.jsonl`),V=gs(X);j=V.userMessage,P=V.assistantMessage}}catch{}if(p.length===0&&L.length===0)return d?.close(),e?`
