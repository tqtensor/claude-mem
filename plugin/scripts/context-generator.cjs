...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let T="";if(n){let{sessionId:g,memorySessionId:f,correlationId:S,...p}=n;Object.keys(p).length>0&&(T=` {${Object.entries(p).map(([b,l])=>`${b}=${l}`).join(", ")}}`)}let E=`[${i}] [${a}] [${d}] ${c}${s}${T}${u}`;if(this.logFilePath)try{(0,L.appendFileSync)(this.logFilePath,E+`
