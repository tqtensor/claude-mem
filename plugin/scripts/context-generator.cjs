...
`).filter(n=>n.trim()),t="";for(let n=s.length-1;n>=0;n--)try{let o=s[n];if(!o.includes('"type":"assistant"'))continue;let d=JSON.parse(o);if(d.type==="assistant"&&d.message?.content&&Array.isArray(d.message.content)){let c="";for(let _ of d.message.content)_.type==="text"&&(c+=_.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){t=c;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return f.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:a},e),{userMessage:"",assistantMessage:""}}}async function rs(a,e=!1){let s=Ze(),t=a?.cwd??process.cwd(),n=$e(t),o=null;try{o=new J}catch(I){if(I.code==="ERR_DLOPEN_FAILED"){try{(0,H.unlinkSync)(ze)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw I}let d=Array.from(s.observationTypes),c=d.map(()=>"?").join(","),_=Array.from(s.observationConcepts),u=_.map(()=>"?").join(","),l=o.db.prepare(`
  ...
  `).all(n,...d,..._,s.totalObservationCount),E=o.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+es),R="",T="";if(s.showLastMessage&&l.length>0){let I=a?.session_id,O=l.find(D=>D.sdk_session_id!==I);if(O){let D=O.sdk_session_id,U=ss(t),N=Z.default.join((0,ee.homedir)(),".claude","projects",U,`${D}.jsonl`),y=ts(N);R=y.userMessage,T=y.assistantMessage}}if(l.length===0&&E.length===0)return o?.close(),e?`
...
No previous sessions found for this project yet.`;let A=E.slice(0,s.sessionCount),g=l,r=[];if(e?(r.push(""),r.push(`${i.bright}${i.cyan}[${n}] recent context${i.reset}`),r.push(`${i.gray}${"\u2500".repeat(60)}${i.reset}`),r.push("")):(r.push(`# [${n}] recent context`),r.push("")),g.length>0){let O=W.getInstance().getActiveMode().observation_types.map(p=>`${p.emoji} ${p.id}`).join(" | ");e?r.push(`${i.dim}Legend: \u{1F3AF} session-request | ${O}${i.reset}`):r.push(`**Legend:** \u{1F3AF} session-request | ${O}`),r.push(""),e?(r.push(`${i.bright}\u{1F4A1} Column Key${i.reset}`),r.push(`${i.dim}  Read: Tokens to read this observation (cost to learn it now)${i.reset}`),r.push(`${i.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${i.reset}`)):(r.push("\u{1F4A1} **Column Key**:"),r.push("- **Read**: Tokens to read this observation (cost to learn it now)"),r.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),r.push(""),e?(r.push(`${i.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${i.reset}`),r.push(""),r.push(`${i.dim}When you need implementation details, rationale, or debugging context:${i.reset}`),r.push(`${i.dim}  - Use the mem-search skill to fetch full observations on-demand${i.reset}`),r.push(`${i.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${i.reset}`),r.push(`${i.dim}  - Trust this index over re-reading code for past decisions and learnings${i.reset}`)):(r.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),r.push(""),r.push("When you need implementation details, rationale, or debugging context:"),r.push("- Use the mem-search skill to fetch full observations on-demand"),r.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),r.push("- Trust this index over re-reading code for past decisions and learnings")),r.push("");let D=l.length,U=l.reduce((p,h)=>{let S=(h.title?.length||0)+(h.subtitle?.length||0)+(h.narrative?.length||0)+JSON.stringify(h.facts||[]).length;return p+Math.ceil(S/Ue)},0),N=l.reduce((p,h)=>p+(h.discovery_tokens||0),0),y=N-U,G=N>0?Math.round(y/N*100):0,_e=s.showReadTokens||s.showWorkTokens||s.showSavingsAmount||s.showSavingsPercent;if(_e)if(e){if(r.push(`${i.bright}${i.cyan}\u{1F4CA} Context Economics${i.reset}`),r.push(`${i.dim}  Loading: ${D} observations (${U.toLocaleString()} tokens to read)${i.reset}`),r.push(`${i.dim}  Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions${i.reset}`),N>0&&(s.showSavingsAmount||s.showSavingsPercent)){let p="  Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?p+=`${y.toLocaleString()} tokens (${G}% reduction from reuse)`:s.showSavingsAmount?p+=`${y.toLocaleString()} tokens`:p+=`${G}% reduction from reuse`,r.push(`${i.green}${p}${i.reset}`)}r.push("")}else{if(r.push("\u{1F4CA} **Context Economics**:"),r.push(`- Loading: ${D} observations (${U.toLocaleString()} tokens to read)`),r.push(`- Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions`),N>0&&(s.showSavingsAmount||s.showSavingsPercent)){let p="- Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?p+=`${y.toLocaleString()} tokens (${G}% reduction from reuse)`:s.showSavingsAmount?p+=`${y.toLocaleString()} tokens`:p+=`${G}% reduction from reuse`,r.push(p)}r.push("")}let xe=E[0]?.id,we=A.map((p,h)=>{let S=h===0?null:E[h+1];return{...p,displayEpoch:S?S.created_at_epoch:p.created_at_epoch,displayTime:S?S.created_at:p.created_at,shouldShowLink:p.id!==xe}}),Fe=new Set(l.slice(0,s.fullObservationCount).map(p=>p.id)),ue=[...g.map(p=>({type:"observation",data:p})),...we.map(p=>({type:"summary",data:p}))];ue.sort((p,h)=>{let S=p.type==="observation"?p.data.created_at_epoch:p.data.displayEpoch,M=h.type==="observation"?h.data.created_at_epoch:h.data.displayEpoch;return S-M});let B=new Map;for(let p of ue){let h=p.type==="observation"?p.data.created_at:p.data.displayTime,S=De(h);B.has(S)||B.set(S,[]),B.get(S).push(p)}let Xe=Array.from(B.entries()).sort((p,h)=>{let S=new Date(p[0]).getTime(),M=new Date(h[0]).getTime();return S-M});for(let[p,h]of Xe){e?(r.push(`${i.bright}${i.cyan}${p}${i.reset}`),r.push("")):(r.push(`### ${p}`),r.push(""));let S=null,M="",x=!1;for(let se of h)if(se.type==="summary"){x&&(r.push(""),x=!1,S=null,M="");let m=se.data,w=`${m.request||"Session started"} (${ve(m.displayTime)})`;e?r.push(`\u{1F3AF} ${i.yellow}#S${m.id}${i.reset} ${w}`):r.push(`**\u{1F3AF} #S${m.id}** ${w}`),r.push("")}else{let m=se.data,w=Me(m.files_modified,t);w!==S&&(x&&r.push(""),e?r.push(`${i.dim}${w}${i.reset}`):r.push(`**${w}**`),e||(r.push("| ID | Time | T | Title | Read | Work |"),r.push("|----|------|---|-------|------|------|")),S=w,x=!0,M="");let F=ye(m.created_at),Y=m.title||"Untitled",V=W.getInstance().getTypeIcon(m.type),je=(m.title?.length||0)+(m.subtitle?.length||0)+(m.narrative?.length||0)+JSON.stringify(m.facts||[]).length,X=Math.ceil(je/Ue),j=m.discovery_tokens||0,te=W.getInstance().getWorkEmoji(m.type),me=j>0?`${te} ${j.toLocaleString()}`:"-",re=F!==M,Ee=re?F:"";if(M=F,Fe.has(m.id)){let k=s.fullObservationField==="narrative"?m.narrative:m.facts?ce(m.facts).join(`
`):null;if(e){let C=re?`${i.dim}${F}${i.reset}`:" ".repeat(F.length),K=s.showReadTokens&&X>0?`${i.dim}(~${X}t)${i.reset}`:"",Te=s.showWorkTokens&&j>0?`${i.dim}(${te} ${j.toLocaleString()}t)${i.reset}`:"";r.push(`  ${i.dim}#${m.id}${i.reset}  ${C}  ${V}  ${i.bright}${Y}${i.reset}`),k&&r.push(`    ${i.dim}${k}${i.reset}`),(K||Te)&&r.push(`    ${K} ${Te}`),r.push("")}else{x&&(r.push(""),x=!1),r.push(`**#${m.id}** ${Ee||"\u2033"} ${V} **${Y}**`),k&&(r.push(""),r.push(k),r.push(""));let C=[];s.showReadTokens&&C.push(`Read: ~${X}`),s.showWorkTokens&&C.push(`Work: ${me}`),C.length>0&&r.push(C.join(", ")),r.push(""),S=null}}else if(e){let k=re?`${i.dim}${F}${i.reset}`:" ".repeat(F.length),C=s.showReadTokens&&X>0?`${i.dim}(~${X}t)${i.reset}`:"",K=s.showWorkTokens&&j>0?`${i.dim}(${te} ${j.toLocaleString()}t)${i.reset}`:"";r.push(`  ${i.dim}#${m.id}${i.reset}  ${k}  ${V}  ${Y} ${C} ${K}`)}else{let k=s.showReadTokens?`~${X}`:"",C=s.showWorkTokens?me:"";r.push(`| #${m.id} | ${Ee||"\u2033"} | ${V} | ${Y} | ${k} | ${C} |`)}}x&&r.push("")}let L=E[0],le=l[0];if(s.showLastSummary&&L&&(L.investigated||L.learned||L.completed||L.next_steps)&&(!le||L.created_at_epoch>le.created_at_epoch)&&(r.push(...z("Investigated",L.investigated,i.blue,e)),r.push(...z("Learned",L.learned,i.yellow,e)),r.push(...z("Completed",L.completed,i.green,e)),r.push(...z("Next Steps",L.next_steps,i.magenta,e))),T&&(r.push(""),r.push("---"),r.push(""),e?(r.push(`${i.bright}${i.magenta}\u{1F4CB} Previously${i.reset}`),r.push(""),r.push(`${i.dim}A: ${T}${i.reset}`)):(r.push("**\u{1F4CB} Previously**"),r.push(""),r.push(`A: ${T}`)),r.push("")),_e&&N>0&&y>0){let p=Math.round(N/1e3);r.push(""),e?r.push(`${i.dim}\u{1F4B0} Access ${p}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${i.reset}`):r.push(`\u{1F4B0} Access ${p}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}return o?.close(),r.join(`
