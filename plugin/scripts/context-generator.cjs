...
`+JSON.stringify(o,null,2)}catch{c=" "+this.formatData(o)}else c=" "+this.formatData(o);let E="";if(n){let{sessionId:g,memorySessionId:b,correlationId:N,...l}=n;Object.keys(l).length>0&&(E=` {${Object.entries(l).map(([f,T])=>`${f}=${T}`).join(", ")}}`)}let p=`[${i}] [${a}] [${d}] ${m}${s}${E}${c}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,p+`
