...
`+JSON.stringify(o,null,2):_=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([S,N])=>`${S}=${N}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${u}${_}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),_=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:_};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},m=new W;var Ct={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(Ct.url))}var Rt=Ot(),A=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),Yt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),Vt=(0,b.join)(A,"archives"),qt=(0,b.join)(A,"logs"),Kt=(0,b.join)(A,"trash"),Jt=(0,b.join)(A,"backups"),zt=(0,b.join)(A,"modes"),Qt=(0,b.join)(A,"settings.json"),_e=(0,b.join)(A,"claude-mem.db"),Zt=(0,b.join)(A,"vector-db"),es=(0,b.join)(A,"observer-sessions"),ts=(0,b.join)(y,"settings.json"),ss=(0,b.join)(y,"commands"),rs=(0,b.join)(y,"CLAUDE.md");function ue(r){(0,ce.mkdirSync)(r,{recursive:!0,mode:448})}function le(){return(0,b.join)(Rt,"..")}var $=class{db;constructor(e=_e){e!==":memory:"&&ue(A),this.db=new Ee.Database(e),e!==":memory:"&&(0,ge.chmodSync)(e,384),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",_=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),g=[...e],E=[];if(o&&(E.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),g.push(...i)}else E.push("type = ?"),g.push(i);if(a){let l=Array.isArray(a)?a:[a],f=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...l),E.push(`(${f.join(" OR ")})`)}if(d){let l=Array.isArray(d)?d:[d],f=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(S=>{g.push(`%${S}%`,`%${S}%`)}),E.push(`(${f.join(" OR ")})`)}let T=E.length>0?`WHERE id IN (${u}) AND ${E.join(" AND ")}`:`WHERE id IN (${u})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${T}
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=this.db.prepare(`
      ...
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       ...
       created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,s.source_tool||null,s.source_input_summary||null,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, source_tool, source_input_summary,
         created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let h=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,T.source_tool||null,T.source_input_summary||null,c,d);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let _=c??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, source_tool, source_input_summary,
         created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,f.source_tool||null,f.source_input_summary||null,u,_);E.push(Number(S.lastInsertRowid))}let h;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,_);h=Number(S.lastInsertRowid)}return this.db.prepare(`
...
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...xe(r));let a=null,d="",c=!1;for(let _ of e)if(_.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let u=_.data,g=rt(u.displayTime);o?i.push(...ze(u,g)):i.push(...$e(u,g))}else{let u=_.data,g=it(u.files_modified,n,u.files_read),E=nt(u.created_at),T=E!==d,h=T?E:"";d=E;let l=t.has(u.id);if(g!==a&&(c&&i.push(""),o?i.push(...qe(g)):i.push(...Ue(g)),a=g,c=!0),l){let f=vt(u,s);o?i.push(...Je(u,E,T,f,s)):(c&&!o&&(i.push(""),c=!1),i.push(...we(u,h,f,s)),a=null)}else o?i.push(Ke(u,E,T,s)):i.push(ke(u,h,s))}return c&&i.push(""),i}function at(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,d]of i)o.push(...Lt(a,d,e,t,s,n));return o}function dt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,p.blue)),t.push(...U("Learned",r.learned,p.yellow)),t.push(...U("Completed",r.completed,p.green)),t.push(...U("Next Steps",r.next_steps,p.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):Fe(r)}function mt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ze(r.totalDiscoveryTokens,r.totalReadTokens):Pe(r.totalDiscoveryTokens,r.totalReadTokens)}var Dt=_t.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function xt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,lt.unlinkSync)(Dt)}catch(e){m.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return m.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?et(r):Xe(r)}function kt(r,e,t,s,n,o,i){let a=[],d=K(e);a.push(...tt(r,d,s,i));let c=t.slice(0,s.sessionCount),_=Ne(c,t),u=Z(e,_),g=Ae(e,s.fullObservationCount);a.push(...at(u,g,s,n,i));let E=t[0],T=e[0];dt(s,E,T)&&a.push(...pt(E,i));let h=Q(e,s,o,n);return a.push(...ct(h,i)),a.push(...mt(d,s,i)),a.join(`
