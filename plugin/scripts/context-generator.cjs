...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",T={...r,location:l};return this.warn(e,`[HAPPY-PATH] ${s}`,T,n),i}},M=new ne;var k=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,he.join)((0,be.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:ge,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:Se,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false",CLAUDE_MEM_ENDLESS_MODE:"false",CLAUDE_MEM_ENDLESS_WAIT_TIMEOUT_MS:"90000"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,X.existsSync)(e))return this.getAllDefaults();let s=(0,X.readFileSync)(e,"utf-8"),r=JSON.parse(s),n=r;if(r.env&&typeof r.env=="object"){n=r.env;try{(0,X.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),M.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){M.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}};var Ge={};function He(){return typeof __dirname<"u"?__dirname:(0,O.dirname)((0,Re.fileURLToPath)(Ge.url))}var us=He(),C=k.get("CLAUDE_MEM_DATA_DIR"),oe=process.env.CLAUDE_CONFIG_DIR||(0,O.join)((0,fe.homedir)(),".claude"),ls=(0,O.join)(C,"archives"),Es=(0,O.join)(C,"logs"),ms=(0,O.join)(C,"trash"),Ts=(0,O.join)(C,"backups"),gs=(0,O.join)(C,"settings.json"),Ne=(0,O.join)(C,"claude-mem.db"),Ss=(0,O.join)(C,"vector-db"),hs=(0,O.join)(oe,"settings.json"),bs=(0,O.join)(oe,"commands"),fs=(0,O.join)(oe,"CLAUDE.md");function Ae(c){(0,Oe.mkdirSync)(c,{recursive:!0})}var q=class{db;constructor(){Ae(C),this.db=new Ie.Database(Ne),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn()}initializeSchema(){try{this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i,type:a,concepts:d,files:p}=s,u=r==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",T=e.map(()=>"?").join(","),b=[...e],m=[];if(i&&(m.push("project = ?"),b.push(i)),a)if(Array.isArray(a)){let t=a.map(()=>"?").join(",");m.push(`type IN (${t})`),b.push(...a)}else m.push("type = ?"),b.push(a);if(d){let t=Array.isArray(d)?d:[d],N=t.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");b.push(...t),m.push(`(${N.join(" OR ")})`)}if(p){let t=Array.isArray(p)?p:[p],N=t.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");t.forEach(R=>{b.push(`%${R}%`,`%${R}%`)}),m.push(`(${N.join(" OR ")})`)}let A=m.length>0?`WHERE id IN (${T}) AND ${m.join(" AND ")}`:`WHERE id IN (${T})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${A}
    ...
    `).run(e,s,r,n.toISOString(),i).lastInsertRowid}getUserPrompt(e,s){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,r,n,i=0,a){let d=new Date,p=d.getTime();this.db.prepare(`
      SELECT id FROM sdk_sessions WHERE sdk_session_id = ?
    `).get(e)||(this.db.prepare(`
        INSERT INTO sdk_sessions
        (claude_session_id, sdk_session_id, project, started_at, started_at_epoch, status)
        VALUES (?, ?, ?, ?, ?, 'active')
      `).run(e,e,s,d.toISOString(),p),console.log(`[SessionStore] Auto-created session record for session_id: ${e}`));let b=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch, tool_use_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,r.type,r.title,r.subtitle,JSON.stringify(r.facts),r.narrative,JSON.stringify(r.concepts),JSON.stringify(r.files_read),JSON.stringify(r.files_modified),n||null,i,d.toISOString(),p,a||null);return{id:Number(b.lastInsertRowid),createdAtEpoch:p}}storeSummary(e,s,r,n,i=0){let a=new Date,d=a.getTime();this.db.prepare(`
      SELECT id FROM sdk_sessions WHERE sdk_session_id = ?
    `).get(e)||(this.db.prepare(`
        INSERT INTO sdk_sessions
        (claude_session_id, sdk_session_id, project, started_at, started_at_epoch, status)
    ...
    `).all(...u)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i}=s,a=r==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),u=[...e],l=i?"AND s.project = ?":"";return i&&u.push(i),this.db.prepare(`
      ...
      ${d}
    `).all(...u)}getTimelineAroundTimestamp(e,s=10,r=10,n){return this.getTimelineAroundObservation(null,e,s,r,n)}getTimelineAroundObservation(e,s,r=10,n=10,i){let a=i?"AND project = ?":"",d=i?[i]:[],p,u;if(e!==null){let m=`
      ...
      `,A=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let g=this.db.prepare(m).all(s,...d,r),t=this.db.prepare(A).all(s,...d,n+1);if(g.length===0&&t.length===0)return{observations:[],sessions:[],prompts:[]};p=g.length>0?g[g.length-1].created_at_epoch:s,u=t.length>0?t[t.length-1].created_at_epoch:s}catch(g){return console.error("[SessionStore] Error getting boundary timestamps:",g.message,i?`(project: ${i})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
    ...
    `;try{let m=this.db.prepare(l).all(p,u,...d),A=this.db.prepare(T).all(p,u,...d),g=this.db.prepare(b).all(p,u,...d);return{observations:m,sessions:A.map(t=>({id:t.id,sdk_session_id:t.sdk_session_id,project:t.project,request:t.request,completed:t.completed,next_steps:t.next_steps,created_at:t.created_at,created_at_epoch:t.created_at_epoch})),prompts:g.map(t=>({id:t.id,claude_session_id:t.claude_session_id,prompt_number:t.prompt_number,prompt_text:t.prompt_text,project:t.project,created_at:t.created_at,created_at_epoch:t.created_at_epoch}))}}catch(m){return console.error("[SessionStore] Error querying timeline records:",m.message,i?`(project: ${i})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}getObservationsByToolUseId(e){return this.db.prepare(`
      SELECT * FROM observations
      WHERE tool_use_id = ?
      ORDER BY created_at_epoch ASC
    `).all(e)}getPromptById(e){return this.db.prepare(`
      SELECT
        p.id,
        p.claude_session_id,
        p.prompt_number,
...
`).filter(n=>n.trim()),r="";for(let n=s.length-1;n>=0;n--)try{let i=s[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let d="";for(let p of a.message.content)p.type==="text"&&(d+=p.text);if(d=d.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),d){r=d;break}}}catch{continue}return{userMessage:"",assistantMessage:r}}catch(e){return M.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:c},e),{userMessage:"",assistantMessage:""}}}async function ze(c,e=!1){let s=Ke(),r=c?.cwd??process.cwd(),n=r?B.default.basename(r):"unknown-project",i=null;try{i=new q}catch(N){if(N.code==="ERR_DLOPEN_FAILED"){try{(0,W.unlinkSync)(Ve)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw N}let a=Array.from(s.observationTypes),d=a.map(()=>"?").join(","),p=Array.from(s.observationConcepts),u=p.map(()=>"?").join(","),l=i.db.prepare(`
  ...
  `).all(n,...a,...p,s.totalObservationCount),T=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+qe),b="",m="";if(s.showLastMessage&&l.length>0)try{let N=c?.session_id,R=l.find(f=>f.sdk_session_id!==N);if(R){let f=R.sdk_session_id,y=Je(r),w=B.default.join((0,Q.homedir)(),".claude","projects",y,`${f}.jsonl`),j=Qe(w);b=j.userMessage,m=j.assistantMessage}}catch{}if(l.length===0&&T.length===0)return i?.close(),e?`
