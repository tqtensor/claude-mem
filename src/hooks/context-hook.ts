...
function getContextDepth(): number {
  ...
  return parseInt(process.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS || happy_path_error__with_fallback('context-hook: CLAUDE_MEM_CONTEXT_OBSERVATIONS not set', {}, '50'), 10);
}
...
async function contextHook(input?: SessionStartInput, useColors: boolean = false): Promise<string> {
  ...
  if (timelineObs.length > 0) {
    ...
    const totalReadTokens = observations.reduce((sum, obs) => {
      ...
      const obsSize = (obs.title?.length || happy_path_error__with_fallback('context-hook: obs.title.length is null', { obsId: obs.id }, 0)) +
                      (obs.subtitle?.length || happy_path_error__with_fallback('context-hook: obs.subtitle.length is null', { obsId: obs.id }, 0)) +
                      (obs.narrative?.length || happy_path_error__with_fallback('context-hook: obs.narrative.length is null', { obsId: obs.id }, 0)) +
                      JSON.stringify(obs.facts || happy_path_error__with_fallback('context-hook: obs.facts is null', { obsId: obs.id }, [])).length;
      return sum + Math.ceil(obsSize / CHARS_PER_TOKEN_ESTIMATE);
    }, 0);
    const totalDiscoveryTokens = observations.reduce((sum, obs) => sum + (obs.discovery_tokens || happy_path_error__with_fallback('context-hook: obs.discovery_tokens is null', { obsId: obs.id }, 0)), 0);
    ...
    for (const [day, dayItems] of sortedDays) {
      ...
      for (const item of dayItems) {
        if (item.type === 'summary') {
          ...
          const summaryTitle = `${summary.request || happy_path_error__with_fallback('context-hook: summary.request is null', { summaryId: summary.id }, 'Session started')} (${formatDateTime(summary.displayTime)})`;
          ...
          const title = obs.title || happy_path_error__with_fallback('context-hook: obs.title is null', { obsId: obs.id }, 'Untitled');
          ...
          const obsSize = (obs.title?.length || happy_path_error__with_fallback('context-hook: obs.title.length is null (timeline)', { obsId: obs.id }, 0)) +
                          (obs.subtitle?.length || happy_path_error__with_fallback('context-hook: obs.subtitle.length is null (timeline)', { obsId: obs.id }, 0)) +
                          (obs.narrative?.length || happy_path_error__with_fallback('context-hook: obs.narrative.length is null (timeline)', { obsId: obs.id }, 0)) +
                          JSON.stringify(obs.facts || happy_path_error__with_fallback('context-hook: obs.facts is null (timeline)', { obsId: obs.id }, [])).length;
          ...
          const discoveryTokens = obs.discovery_tokens || happy_path_error__with_fallback('context-hook: obs.discovery_tokens is null (timeline)', { obsId: obs.id }, 0);
          ...
          if (useColors) {
            ...
            output.push(`| #${obs.id} | ${timeDisplay || happy_path_error__with_fallback('context-hook: timeDisplay is null', { obsId: obs.id }, '″')} | ${icon} | ${title} | ~${readTokens} | ${discoveryDisplay} |`);
          }
        }
      }
