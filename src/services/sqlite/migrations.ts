...
/**
 * Migration 008 - Add mode column and remove type CHECK constraint
 * Enables mode-agnostic observations (support for code, email-investigation, etc.)
 */
export const migration008: Migration = {
  ...
  up: (db: Database) => {
    ...
    // 1. Create new table without CHECK constraint and with mode column
    db.run(`
    ...
    // 2. Copy all data from old table (mode will be NULL for existing records = 'code' mode)
    db.run(`
      INSERT INTO observations_new (
        id, sdk_session_id, project, text, type, mode,
        title, subtitle, narrative, facts, concepts, files_touched,
        discovery_tokens, created_at, created_at_epoch
      )
      SELECT
        id, sdk_session_id, project, text, type, NULL,
        title, subtitle, narrative, facts, concepts, files_touched,
        discovery_tokens, created_at, created_at_epoch
      FROM observations
    `);
    ...
    db.run(`
      ...
      CREATE INDEX idx_observations_mode ON observations(mode);
    ...
    console.log('✅ Added mode column and removed type CHECK constraint for mode-agnostic observations');
  },

  down: (db: Database) => {
    // Recreate observations table with CHECK constraint
    db.run(`
      CREATE TABLE observations_new (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sdk_session_id TEXT NOT NULL,
    ...
    db.run(`
      INSERT INTO observations_new (
        id, sdk_session_id, project, text, type,
        title, subtitle, narrative, facts, concepts, files_touched,
        discovery_tokens, created_at, created_at_epoch
      )
      SELECT
    ...
    console.log('⚠️  Rolled back to type CHECK constraint (mode column removed)');
  }
};
