  ...
  private async processOpenRouterResponse(
    ...
    for (const obs of observations) {
      ...
      this.dbManager.getChromaSync().syncObservation(
        ...
        logger.warn('SDK', 'OpenRouter chroma sync failed', { obsId: result.id }, err);
      });
    ...
    if (summary) {
      ...
      this.dbManager.getChromaSync().syncSummary(
        ...
        logger.warn('SDK', 'OpenRouter chroma sync failed', { summaryId: result.id }, err);
      });
    ...
    if (observationIds.length > 0 || summaryId) {
      logger.info('SDK', 'OpenRouter observations and summary saved', {
        sessionId: session.sessionDbId,
