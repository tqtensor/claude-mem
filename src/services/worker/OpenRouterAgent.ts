...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      await this.executeOpenRouterSession(session, worker);
    } catch (error: unknown) {
  ...
  /**
   * Execute the OpenRouter session lifecycle: init prompt, message loop, completion.
   * Extracted from startSession to keep the try-catch block small.
   */
  private async executeOpenRouterSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
