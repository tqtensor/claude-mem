...
export class GeminiAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      await this.executeGeminiSession(session, worker);
    } catch (error: unknown) {
  ...
  /**
   * Execute the Gemini session lifecycle: init prompt, message loop, completion.
   * Extracted from startSession to keep the try-catch block small.
   */
  private async executeGeminiSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
