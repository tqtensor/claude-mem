...
export class SearchManager {
  ...
  async search(args: any): Promise<any> {
    ...
    // PATH 2: CHROMA SEMANTIC SEARCH (query text + Chroma available)
      ...
      // Include branch in the Chroma where clause to scope vector search to a specific branch.
      if (options.branch) {
        const branchFilter = { branch: options.branch };
        whereFilter = whereFilter
          ? { $and: [whereFilter, branchFilter] }
          : branchFilter;
      }

      // Step 1: Chroma semantic search with optional type + project + branch filter
  ...
  async timeline(args: any): Promise<any> {
    const { anchor, query, depth_before = 10, depth_after = 10, project, branch } = args;
    const cwd = process.cwd();
    ...
    if (query) {
      ...
      timelineData = this.sessionStore.getTimelineAroundObservation(topResult.id, topResult.created_at_epoch, depth_before, depth_after, project, branch);
    }
    // MODE 2: Anchor-based timeline
      ...
      timelineData = this.sessionStore.getTimelineAroundObservation(anchor, anchorEpoch, depth_before, depth_after, project, branch);
      ...
      if (anchor.startsWith('S') || anchor.startsWith('#S')) {
        ...
        timelineData = this.sessionStore.getTimelineAroundTimestamp(anchorEpoch, depth_before, depth_after, project, branch);
      } else {
        // ISO timestamp
  ...
  async getContextTimeline(args: any): Promise<any> {
    const { anchor, depth_before = 10, depth_after = 10, project, branch } = args;
    const cwd = process.cwd();
    let anchorEpoch: number;
  ...
  async getTimelineByQuery(args: any): Promise<any> {
    const { query, mode = 'auto', depth_before = 10, depth_after = 10, limit = 5, project, branch } = args;
    const cwd = process.cwd();
